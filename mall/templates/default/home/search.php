...
.m-related{margin-bottom:10px;}
.m-related .inner{height:15px;overflow:hidden;padding:12px 19px;color:#9a9a9a;border:1px solid #e8e8e8 }
.m-related .no-border{border:none}
.m-related .title{float:left;color:#9a9a9a;white-space:nowrap}
.m-related .item{height:18px;overflow:hidden}
.m-related .link{float:left;color:#6c6c6c;margin-left:20px}
.m-related .link:hover{color:#F40}.m-sortbar{margin-top:10px}
#checkerror {padding: 15px 10px;border: 1px solid #EDD28B;background: none repeat scroll 0% 0% #FFFDEE;font-size: 14px;font-weight: 700;}
...
<div class="nch-container wrapper" >
  ...
  <div class="right"> 
  <!--  错误搜索开始qunashop-->
  <?php  if(!empty($output['index_corrected'])) { ?>
  <div class="m" id="checkerror">
	我们为您显示"<span><?php echo $output['index_corrected'];?></span>"相关的商品。仍然搜索："<span><a href="<?php echo SHOP_SITE_URL;?>/index.php?con=search&fun=index&keyword=<?php echo $_GET['keyword'];?>&qrst=UNEXPAND"><?php echo $_GET['keyword'];?></a></span>"
</div>
<?php } ?>
  <!--  错误搜索结束-->  	
    <!-- 分类下的推荐商品 -->
    ...
    <div class="shop_con_list" id="main-nav-holder">
      <!--你是不是想找开始qunashop-->
    <?php  if(!empty($output['indexer_related'])) { ?>  
      <div id="mainsrp-related"><div class="m-related">
  <dl class="inner ">
    <dt class="title">您是不是想找：</dt>
    <dd class="item">
    <?php foreach ($output['indexer_related'] as $vae) {?>
        <a class="link " trace="relatedSearch" href="<?php echo SHOP_SITE_URL;?>/index.php?con=search&fun=index&keyword=<?php echo $vae;?>"><?php echo $vae;?></a> 
        <?php } ?>    
    </dd>
  </dl>
</div>
