  ...
  <div class="joinin-pay">
    ...
    <table border="0" cellpadding="0" cellspacing="0" class="store-joinin">
      ...
      <tr>
        ...
        <td colspan="20"><?php echo $output['joinin_detail']['legal_person_name'];?></td>
      </tr>
      <tr>
        <th>公司法人身份证号：</th>
        <td><?php echo $output['joinin_detail']['id_number'];?></td>
      </tr>
      <tr>
        <th>公司法人身份证电子版：</th>
        <td>
          <?php foreach ($output['joinin_detail']['identity_card_electronic'] as $pic_url){ ?>
    ...
    <form id="form_paying_money_certificate" action="index.php?con=store_joinin&fun=pay_save" method="post" enctype="multipart/form-data">
      ...
      <table border="0" cellpadding="0" cellspacing="0" class="store-joinin">
        ...
        <tbody>
          ...
          <tr>
            <th>质检报告扫描件或产品质量合格证明:</th>
            <td colspan="20">
              <?php foreach($output['joinin_detail']['quality_report'] as $pic_url){?>
              <a nctype="nyroModal"  href="<?php echo getStoreJoininImageUrl($pic_url);?>"><?php if(!empty($pic_url)){ ?> <img src="<?php echo getStoreJoininImageUrl($pic_url);?>" alt="" /> </a>
              <?php } ?><?php } ?>
            </td>
          </tr>
          <tr>
            <th>商标注册证:</th>
            <td colspan="20">
              <?php foreach($output['joinin_detail']['trade_mark_license'] as $pic_url){?>
              <a nctype="nyroModal"  href="<?php echo getStoreJoininImageUrl($pic_url);?>"><?php if(!empty($pic_url)){ ?> <img src="<?php echo getStoreJoininImageUrl($pic_url);?>" alt="" /> </a>
              <?php } ?><?php } ?>
            </td>
          </tr>
          <tr>
            <th>产品授权书:</th>
            <td colspan="20">
              <?php foreach($output['joinin_detail']['authorization'] as $pic_url){?>
              <a nctype="nyroModal"  href="<?php echo getStoreJoininImageUrl($pic_url);?>"><?php if(!empty($pic_url)){ ?> <img src="<?php echo getStoreJoininImageUrl($pic_url);?>" alt="" /> </a>
              <?php } ?><?php } ?>
            </td>
          </tr>
          <tr>
            <th>其他资质栏:</th>
            <td colspan="20">
              <?php foreach($output['joinin_detail']['other_qualifications'] as $pic_url){?>
              <a nctype="nyroModal"  href="<?php echo getStoreJoininImageUrl($pic_url);?>"><?php if(!empty($pic_url)){ ?> <img src="<?php echo getStoreJoininImageUrl($pic_url);?>" alt="" /> </a>
              <?php } ?><?php } ?>
            </td>
          </tr>
      ...
      <table border="0" cellpadding="0" cellspacing="0" class="store-joinin">
        <tbody>
          ...
          <tr>
            <th>缴费账号：</th>
            <td >账户名:&nbsp&nbsp冠芳电子商务有限公司<br/>
              银行卡号:<strong>&nbsp&nbsp6222&nbsp5313&nbsp0201&nbsp6222 </strong><br/>
              支行名称:&nbsp&nbsp中国建设银行股份有限公司西藏支行<br/>
            </td>
