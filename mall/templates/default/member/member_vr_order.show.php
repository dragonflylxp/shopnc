...
<div class="ncm-oredr-show">
  <div class="ncm-order-info">
    ...
    <div class="ncm-order-condition">
      ...
      <?php if ($output['order_info']['payment_code'] == 'offbank') { ?>
      <div style="margin-left: 41px;">
        <p style="font-size:14px;font-weight: bold;">您选择了银行汇款支付</p>
        <p class="save_sn" style="font-size:13px;line-height: 28px;<?php if ($output['order_info']['trade_no']!='') { ?>display: none;<?php } ?>">请填写汇款单号：<input id="trade_no" type="text" size="36" value="<?php echo $output['order_info']['trade_no'];?>" /><span onclick="save_sn();" style="background: #fb6e52;border-radius:2px;color: #fff;padding: 2px 5px;font-size: 12px;margin-left: 3px;cursor: pointer; ">确定</span></p>
        <p class="change_sn" style="font-size:13px;line-height: 28px;<?php if ($output['order_info']['trade_no']=='') { ?>display: none;<?php } ?>">汇款单号：<?php echo $output['order_info']['trade_no'];?>，等待商家确认<span onclick="change_sn();" style="background:#75bc30;border-radius:2px;color: #fff;padding: 2px 5px;font-size: 12px;margin-left: 3px;cursor: pointer; ">修改</span></p>
      </div>
    </div>
   <?php } ?>
...
function save_sn(){
  ...
  $.getJSON('index.php?con=member_vr_order&fun=ajaxSaveTradeNo&trade_no='+trade_no+'&order_id='+<?php echo $output['order_info']['order_id'];?>,function(data){
    if(data){
      location.reload();
    }else{
      alert("修改失败");
...
//保存汇款人
function save_sn_name(){
  var trade_no_name = $("#trade_no_name").val();
  if(trade_no_name == ''){
    alert('未填写银行汇款人姓名');
    return;
  }
  $.getJSON('index.php?con=member_vr_order&fun=ajaxSaveTradeNoName&trade_no_name='+trade_no_name+'&order_id='+<?php echo $output['order_info']['order_id'];?>,function(data){
    if(data){
      location.reload();
    }else{
      alert("修改失败");
