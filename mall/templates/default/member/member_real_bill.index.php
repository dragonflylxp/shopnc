...
<div class="wrap">
    ...
    <form method="get" action="index.php" target="_self">
        <table class="ncm-search-table">
            <input type="hidden" name="con" value="member_real_bill"/>
            ...
            <tr>
                ...
                <td class="w100"><select name="state_type">
                        <option
                            value="" <?php echo $_GET['state_type'] == '' ? 'selected' : ''; ?>><?php echo $lang['member_apply_all']; ?></option>
                        <option value="state_new" <?php echo $_GET['state_type'] == 'state_new' ? 'selected' : ''; ?>>
                            未转入
                        </option>
                        <option
                            value="state_payout" <?php echo $_GET['state_type'] == 'state_payout' ? 'selected' : ''; ?>>
                            已转入
                        </option>
                    </select></td>
                ...
                <td class="w240"><input type="text" class="text w70" name="query_start_date" id="query_start_date"
                            ...
                            class="icon-calendar"></i></label>&nbsp;&#8211;&nbsp;<input type="text" class="text w70"
                                                                                        name="query_end_date"
                                                                                        id="query_end_date"
                                                                                        value="<?php echo $_GET['query_end_date']; ?>"/><label
                        class="add-on"><i class="icon-calendar"></i></label></td>
                <td class="w240 tr"><input type="text" class="text w200" placeholder="请输入店铺名称" name="keyword"
                                           value="<?php echo $_GET['keyword']; ?>"></td>
    ...
    <table class="ncm-default-table order">
        ...
        <?php if ($output['member_real_bill']) { ?>
            <tbody>
            <?php foreach ($output['member_real_bill'] as $bill_info) { ?>
                <tr>
                    <?php if ($bill_info['member_ob_state'] == 1) { ?>
                        <td><input type="checkbox" class="check"
                                   data-value="<?php echo $bill_info['member_ob_id']; ?>"/></td>
                    <?php } else { ?>
                    ...
                    <?php if ($bill_info['member_ob_state'] == 1) { ?>
                        <td>
                            <span>
                                <div class="btn3 apply_change"
                                       data-value="<?php echo $bill_info['member_ob_id']; ?>">转入余额
                                </div>
                            </span>
                        </td>
...
<script type="text/javascript">
    $(function () {
        ...
        $('.apply_change').on('click', function () {
            ...
            $.ajax({
                type: "POST",
                url: '<?php echo urlShop('member_real_bill','chageState');?>',
                async: true,
                dataType: "json",
                data: {id: _ob_id},
       ...
       var _a = $(".check:checked").length;
        ...
        $.ajax({
            ...
            success: function(data){
                if(data){
                    for(var i=0;i<_a;i++){
                        obj_arr[i].find('.apply_change').text('已转入').css('color','#666eaf').css('cursor','').off("click");
        ...
        $('#allTobalance').on('mouseover', function () {
            $(this).css('cursie')
        })
    });
</script> 
