    ...
    <ul>
        <li>
            <?php if(!empty($output['dhq_list'])){;?>
                <input type="radio" name="pay_name" class="pay_name" value="online" checked>
            <?php };?>
            在线支付
            <?php if(!empty($output['dhq_list'])){;?>
                <input type="radio" class="pay_name" name="pay_name" value="dhq">兑换卷支付
            <?php };?>
        </li>
        <?php if(!empty($output['dhq_list'])){;?>
        <li>
            <p style="letter-spacing: 1px;color: #888;padding: 0!important;height: 26px;line-height: 36px " class="dhq_msg">如使用兑换卷兑换方式,请在 (我的订单--用户设置--我的代金券) 里面确认自己是否拥有兑换卷。</p>
        </li>
        <?php };?>
    </ul>
  </div>
