...
<form action="<?php echo urlShop('buy_virtual','buy_step3');?>" method="POST" id="form_buy" name="form_buy">
  ...
  <input type="hidden" name="quantity" value="<?php echo $output['goods_info']['quantity'];?>">
    <?php if(!empty($output['dhq_list'])){;?>
        <div style="width: 100%">
            <ul>
                <span style="margin: 0 auto;border-bottom: 1px solid #E6E6E6;display: block;width: 97%;font-size: 16px;color: black;height: 36px;line-height: 36px">支付方式</span>
                <li style="width: 97%;line-height: 36px;height: 36px;margin: 0 auto;border-bottom: 1px solid #E6E6E6">
                    <input type="radio" name="pay_name" value="" checked>在线支付
                    <input type="radio" name="pay_name" value="dhq" style="margin-left: 20px">兑换卷兑换
                </li>
                <li style="width: 97%;line-height: 36px;height: 36px;margin: 0 auto;">
                    <p style="letter-spacing: 1px;color: #888" class="dhq_msg">如使用兑换卷兑换方式,请在 (我的订单--用户设置--我的代金券) 里面确认自己是否拥有兑换卷。</p>
                </li>
            </ul>
        </div>
    <?php };?>

    <input type="hidden" name="pay_name" value="" class="pay_name">
...
$(document).ready(function(){
    ...
    $("input[name='pay_name']").live('change',function(){
        $('.pay_name').val($("input[name='pay_name']:checked").val());
    })

    $('.dhq_msg').mouseover(function(){
        $(this).css('color','#2fc4ff')
    })
    $('.dhq_msg').mouseout(function(){
        $(this).css('color','#888')
    })
