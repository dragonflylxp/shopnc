...
class store_zhiboControl extends BaseHomeControl {
	/**
	 * 店铺列表
	 */
	public function indexOp(){
        ...
        foreach($store_list as $kki=>$vvi){
 
        	$cssd = file_exists(BASE_UPLOAD_PATH.DS.ATTACH_STORE.DS.$vvi['store_id'].DS."zb_show.jpg");

		    if($cssd){
				$store_list[$kki]['img'] = UPLOAD_SITE_URL.DS.ATTACH_STORE.DS.$vvi['store_id'].DS."zb_show.jpg";
		    }else{
				$store_list[$kki]['img'] = UPLOAD_SITE_URL.DS.ATTACH_COMMON.DS.C('default_store_logo');
			}
		...
		// 页面输出
		Tpl::output('index_sign','store_zhibo');
		//当前位置
		...
		Tpl::output('html_title',(empty($_GET['keyword']) ? '' : $_GET['keyword'].' - ').C('site_name')."直播购物-店铺网络直播");
        Tpl::showpage('store_zhibo_home');
	}
