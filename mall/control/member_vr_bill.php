...
class member_vr_billControl extends BaseMemberControl{
    ...
    public function indexOp(){
        ...
        $model_bill = Model('member_vr_bill');
        //搜索
        if ($_GET['keyword'] != '') {
        ...
        $bill = $model_bill->getMemberVBillList($condition);
        self::profile_menu('vr_bill','vr_bill');
        Tpl::output('member_vr_bill',$bill);
        Tpl::output('show_page',$model_bill->showpage());
        Tpl::showpage('member_vr_bill.index');
    }
    /**
     * 点击审核改变状态
     * @throws Exception
    ...
    public function chageStateOp(){
        ...
        for($i=0;$i<$length;$i++){
            ...
            $condition['member_ob_state'] = 1;
            $model_bill = Model('member_vr_bill');
            $result =$model_bill->getOrderBillInfo($condition);
            if($result){
    ...
    public function toBalanceOp($id,$amount){
        ...
        try {
            ...
            $data['pdr_payment_name'] = '虚拟分享收益转入';//支付方式
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'vr_bill':
                $menu_array = array(
                    1=>array('menu_key'=>'vr_bill','menu_name'=>'虚拟结算',   'vr_bill'=>'index.php?con=member_vr_bill&fun=index'));
                break;
        }
