...
class Control{
	...
	protected $city_gps = array();
	protected $store_gps = array();   
    /**
     * 检查短消息数量
     *
     */
    ...
    protected function showGps() {
        ...
        if(empty($_COOKIE['city'])){
		    $json = $this->baiDugps();
		    $jsonr=$json->content->address_detail->city; 
		    $city_point['x']=$json->content->point->x; 
		    $city_point['y']=$json->content->point->y;
		    if(!empty($city_point)){
		    	$area_point = serialize($city_point);
		    	setcookie('citypoint',$area_point,time()+3600*24*30);	
		    	$this->city_gps=$city_point;	
		    } 
		    ...
		    if (!empty($jsonr)) {
		        ...
		        $cond['area_name']= array('like', '%' . $jsonr . '%');   
		        $area_model = Model('area')->getAreaInfo($cond);   
		        if(!empty($area_model)){
			        $area_str = serialize($area_model);
			        setcookie('city',$area_str,time()+3600*24*30);		
			        $this->city_info = $area_model;	
		        }
		    }    	
        }else{ 
		...
		if(empty($this->city_info)){
			$defa = Model('setting')->getRowSetting('default_area');
		   	$cityinfo = unserialize($defa['value']);	
		   		   	
			$this->city_info = $cityinfo;
		}
		
		if(empty($this->city_gps)){			
		   	$citypoint = unserialize($_COOKIE['citypoint']);   	
			
		}else{
			$citypoint = $this->city_gps;
		}

		$defa = Model('setting')->getRowSetting('default_area');	 
		$deep = unserialize($defa['value']);
		$comn['area_id'] = $deep['area_parent_id'];
		$zeep = Model('area')->getAreaInfo($comn);		
		$this->showStore($citypoint); 
		Tpl::output('deep_id',$zeep);
		Tpl::output('dq_id',$deep);
		Tpl::output('city_id',$this->city_info['area_id']);
		Tpl::output('city',$this->city_info['area_name']);	 
		   	
    }
    /**
     * baidu定位请求
     */
    protected function baiDugps() {
        $getIp = $_SERVER["REMOTE_ADDR"];        	
 		$url = "http://api.map.baidu.com/location/ip?ip={$getIp}&ak=066fedaca9ec8ee99b70b7743b1a0b75&coor=bd09ll"; 
 		$ch = curl_init(); 
     ...
     * 定位显示周边店铺
     */  
    protected function showStore($citypoint) {
    	$area_name = $this->city_info['area_name'];
    	$condition['baidu_city'] = array('like', '%' . $area_name . '%');
    	$store_info = Model('store_map')->getStoreMapList($condition);
    	//经纬度为空的话从新拉取定位
    	if(empty($citypoint)){
    		$citypoint = $this->baiDugps();
		    $city_point['x']=$citypoint->content->point->x; 
		    $city_point['y']=$citypoint->content->point->y;
    		$area_point = serialize($city_point);
		    setcookie('citypoint',$area_point,time()+3600*24*30);	
		    $this->city_gps=$city_point;	
    	...
    	//获取位置信息
        if (!empty($store_info)) {
           $this->store_gps = $store_info;
           foreach ($store_info as $key => $value) {
           	    if(!empty($value['baidu_lng']) || !empty($value['baidu_lat'])){

				    //获取用户坐标			 
				    $lngx=$citypoint['x'];//按层级关系提取经度数据
				    $laty=$citypoint['y'];//按层级关系提取纬度数据
				    $zbjl = $this->GetDistance($value['baidu_lat'], $value['baidu_lng'], $laty, $lngx);				
             	    $simply_store_list[$key]['store_id'] = $store_info[$key]['store_id'];
                    $simply_store_list[$key]['store_name'] = $store_info[$key]['store_name'];				
				    //计算坐标距离
				    $simply_store_list[$key]['distance']='距离'.number_format($zbjl, 2, '.', '').'公里';
				    $simply_store_list[$key]['store_info']=$value;				
           	    }else{
             	    $simply_store_list[$key]['store_id'] = $store_info[$key]['store_id'];
                    $simply_store_list[$key]['store_name'] = $store_info[$key]['store_name'];				
				    //计算坐标距离
				    $simply_store_list[$key]['distance']='暂无';
				    $simply_store_list[$key]['store_info']=$value;	           	    	
           	    }           	           	
           }
           Tpl::output('store_gps',$simply_store_list);	
        }else{
            $this->store_gps = '';
        	Tpl::output('store_gps',$this->store_gps);	
        }
...
/********************************** 管理人control父类 *******by suijiaolong***************************************/

class ManagerControl extends Control {
    ...
    protected function checkLoginManager(){
        if ($_SESSION['is_manager_login'] == '1'){
            return true;
        }else {
            @redirect(urlMember('manager_login','index'));die;
        }
    ...
    private function _getMenuList() {
        $menu_list = array(
            ...
            'info' => array('name' => '会员资料', 'child' => array(
                ...
                'member_trustname'   => array('name' => '实名认证', 'url'=>urlMember('member_trustname', 'index')),
                'member_address'    => array('name' => '收货地址', 'url'=>urlMember('member_address', 'address')),
                'member_banks'    => array('name' => '我的银行卡', 'url'=>urlMember('member_banks', 'banks'))
            )),
            ...
            'return_cash' => array('name' => '推荐店铺返现', 'child' => array(
                'member_real_bill'        => array('name' => '实物结算', 'url'=>urlShop('member_real_bill', 'index')),
                'member_vr_bill'    => array('name' => '虚拟结算', 'url'=>urlShop('member_vr_bill', 'index'))
            )),
        );
        return $menu_list;
    }
 ...
 * by：511613932
 */
class VisualSellerControl extends Control {
    //店铺信息
    ...
    private function _getMenuList() {
        $menu_list = array(
            ...
            'store' => array('name' => '店铺', 'child' => array(
                ...
                array('name' => '店铺可视化装修', 'con'=>'store_visual', 'fun'=>'decoration_setting'),
                array('name' => '手机店铺装修', 'con'=>'mb_store_decoration', 'fun'=>'decoration_setting'),
