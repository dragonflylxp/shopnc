...
class store_settingControl extends BaseSellerControl {
	...
	public function store_zhiboOp(){
		...
		if (chksubmit()){
			 ...
			 * 上传店铺图片
			 */
			if (!empty($_FILES['zb_img']['name'])){
				//删除旧图片
				@unlink(BASE_UPLOAD_PATH.DS.ATTACH_STORE.DS.$store_info['store_id'].DS."zb.show.jpg");
				$upload->set('default_dir',	ATTACH_STORE.'/'.$store_info['store_id']);
				$upload->set('thumb_ext',	'');
				$upload->set('file_name','zb_show.jpg');
				$upload->set('ifremove',false);
				$result = $upload->upfile('zb_img');
				if ($result){
					$_POST['zb_img'] = $upload->file_name;
				}else {
					showDialog($upload->error);
				}
			}
			 ...
			 */
			 $z=0;
			 for($i=0;$i<=5;$i++){
				  if($_POST['stream'.$i]){$stream[$z]=$_POST['stream'.$i];$z++;}
				 }
			
			$stream=serialize($stream);
            $param = array(
                'zb_uid' => empty($_POST['zb_uid']) ? $store_info['member_id'] : $_POST['zb_uid'],
                'recode' => $stream,
            );
            if (!empty($_POST['store_theme'])){
                $param['store_theme'] = $_POST['store_theme'];
			...
			//店铺名称修改处理 v3-b12
			if(!empty($_POST['zb_uid']))
			{
				$where=array();
				$where['sid']=$store_id;
				$update=array();
				$update['uid']=empty($_POST['zb_uid'])?$store_info['member_id'] : $_POST['zb_uid'];
				$update['gonggao']=$_POST['zb_gonggao'];
				$update['recode']=$stream;
				$update['bgid']=empty($_POST['zb_bg'])?"4": $_POST['zb_bg'];
				
				$bllGoods=Model()->table('zb_config')->where($where)->update($update);
			}
        }
		$store_info = $model_class->getStoreInfoByID($store_id);
		$zb_config=Model('zhibo')->zb_config($store_id);
		$recode=unserialize($zb_config['recode']);
		
		$cssd = file_exists(BASE_UPLOAD_PATH.DS.ATTACH_STORE.DS.$store_info['store_id'].DS."zb_show.jpg");

		if($cssd){
			$zb_config['img'] = UPLOAD_SITE_URL.DS.ATTACH_STORE.DS.$store_info['store_id'].DS."zb_show.jpg";
		}else{
			$zb_config['img'] = UPLOAD_SITE_URL.DS.ATTACH_COMMON.DS.C('default_store_logo');
		}
        /**
         * 实例化店铺等级模型
         */
		 ...
		 * 输出店铺信息
		 */
		self::profile_menu('store_zhibo');
		Tpl::output('store_info',$store_info);
		Tpl::output('zb_config',$zb_config);
		Tpl::output('recode',$recode);
		/**
		 * 页面输出
		 */
		Tpl::showpage('store_zhibo_form');
	}
    ...
    private function profile_menu($menu_key='') {
        ...
        $menu_array = array(
            ...
            4=>array('menu_key'=>'store_slide','menu_name'=>Language::get('nc_member_path_store_slide'),'menu_url'=>'index.php?con=store_setting&fun=store_slide'),
            ...
            8=>array('menu_key' => 'store_zhibo','menu_name' => '店铺直播设置','menu_url' => 'index.php?con=store_setting&fun=store_zhibo')
        );
