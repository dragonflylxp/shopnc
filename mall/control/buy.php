...
class buyControl extends BaseBuyControl {
    ...
    public function buy_step1Op() {
        ...
        $dhq_list = $db->getExchangeCardByMemberId($_SESSION['member_id'],true);
        Tpl::output('dhq_list',$dhq_list);
    ...
    public function payOp() {
        ...
        if (empty($pay['pay_amount_online']) && empty($pay['pay_amount_offline'])) {
            redirect('index.php?con=buy&fun=pay_ok&pay_sn='.$pay_sn.'&is_chain='.$flag_chain.'&pay_amount='.ncPriceFormat($order_info['order_amount']));
        }
    ...
    public function check_pd_pwdOp(){
        ...
        echo ($buyer_info['member_paypwd'] != '' && $buyer_info['member_paypwd'] === md5($_GET['password']+'XMzDdG7D94CKm1IxIWQw6g==')) ? '1' : '0';
    }
