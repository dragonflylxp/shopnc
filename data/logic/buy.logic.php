...
//             $this->_logic_buy_1->parseZhekou($goods_info,$orderdiscounts);
    ...
    private function _createOrderStep1() {
        ...
        if (!in_array($post['pay_name'],array('online','offline','chain','dhq'))) {
        ...
        if ($post['jforderstr']) {
            ...
            $jforders = explode('#', $post['jforderstr']);
            foreach($jforders as $k=>$jforder){
                $jfinfo = explode('|', $jforder);
                $input_jf_info[$k]['type']   = $jfinfo[0];
                $input_jf_info[$k]['points'] = $jfinfo[1];
                $input_jf_info[$k]['amount'] = $jfinfo[2];
            }
        }
        ...
        $this->_order_data['input_jf_info'] = $input_jf_info;
        $this->_order_data['jforderstr'] = $post['jforderstr'];
    ...
    private function _createOrderStep4() {
        ...
        if ($input_pay_name == 'chain' && $input_chain_id) {
            ...
            foreach ($store_pay_type_list as $k => $v) {
                if($store_pay_type_list[$k] == 'dhq'){
                    $store_pay_type_list[$k] = 'dhq';
                }else{
        ...
        //记录积分抵扣
        $order_pay['jforderstr'] = $jforderstr; 
...
//             $should_goods_total = $store_final_order_total[$store_id]-$store_freight_total[$store_id]+$promotion_total;
            ...
            $order['store_merchantno'] = $store_info['store_merchantno'];
...
//             }
    ...
    private function _createOrderStep5() {
        ...
        if ($buyer_info['member_paypwd'] == '' || $buyer_info['member_paypwd'] != md5($this->_post_data['password']+'XMzDdG7D94CKm1IxIWQw6g==')) return ;
