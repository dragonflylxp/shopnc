...
class queueLogic {
    ...
    public function delCart($cart) {
        ...
        if (empty($cart['cart_ids'])){
            $del = Model('cart')->delCart('db',array('buyer_id'=>$cart['buyer_id']));
        }else{
    ...
    /*******************一次性修改商品二维码  start   @author liming**************************/
     ...
     * @author liming
     */
    public function updateCreateGoodsQRCode($param) {
        ...
        foreach($param['store_ids'] as $store_id ){
            //延迟执行
            sleep(1);
            $PhpQRCode->set('pngTempDir',BASE_UPLOAD_PATH.DS.ATTACH_STORE.DS.$store_id['store_id'].DS);
            foreach ($param['goods_ids'] as $goods_id) {
                if($store_id['store_id']==$goods_id['store_id']){
                    // 生成商品二维码
                    $PhpQRCode->set('date', WAP_SITE_URL.'/tmpl/product_detail.html?goods_id='.$goods_id['goods_id']);
                    $PhpQRCode->set('pngTempName', $goods_id['goods_id'] . '.png');
                    $PhpQRCode->init();
                }
            }
        }
    ...
    public function sendVrCode($param) {
        ...
        if(C('sms.smsNumber') == 2){       //511613932   
        	...
        	$message    = $tpl_info['content'];
        	$par['goodsname']  = $param['goods_name'];
        	$par['vrcode']  =  rtrim($content,',');
        	$par['template']  = C('dysms.code');//511613932
        }	
        
        $sms = new Sms();
        $result = $sms->send($param["buyer_phone"],$message,$par);
        if (!$result) {
    ...
    public function sendPickupcode($param) {
        ...
        $par = array();   //511613932   
        if(C('sms.smsNumber') == 2){    
        	$message = ncReplaceText($tpl_info['content'],$data);
        }else{
        	$message    = $tpl_info['content'];
        	$par['pickupcode']  = $param['pickup_code'];
        	$par['template']  = C('dysms.pickup');//511613932
        }	        
        $sms = new Sms();
        $result = $sms->send($dorder_info['reciver_mobphone'],$message,$par);
        if (!$result) {
    ...
    public function sendChainCode($order_info) {
        ...
        if(C('sms.smsNumber') == 2){    
        	...
        	$message    = $tpl_info['content'];
        	$par['chaincode']  = $order_info['chain_code'];
        	$par['ordersn']  = $order_info['order_sn'];
        	$par['template']  = C('dysms.chaincode');//511613932
        }	                
        $sms = new Sms();
        $result = $sms->send($order_info['buyer_phone'],$message,$par);
        if (!$result) {
