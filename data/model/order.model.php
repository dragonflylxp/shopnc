...
class orderModel extends Model {
    /*
     *
     * 获取已填写银行汇款单号后台还未处理订单条数
     * */
    public function getPayOffbankCount(){
        $condition['payment_code'] = "offbank";
        $condition['payment_time'] = 0;
        $condition['trade_no'] = array('neq','');;
        return $this->table('orders')->where($condition)->count();
    }
    /*
     *
     * 获取已填写银行汇款单号后台还未处理虚拟订单条数
     * */
    public function getVrPayOffbankCount(){
        $condition['payment_code'] = "offbank";
        $condition['payment_time'] = 0;
        $condition['trade_no'] = array('neq','');;
     ...
     * 保存银行汇款单号
     * */
    public function saveTradeNo($condition,$update) {
        return $this->table('orders')->where($condition)->update($update);
    }

    /*
 * 保存虚拟订单银行汇款单号
 * */
    public function saveVrTradeNo($condition,$update) {
        return $this->table('vr_order')->where($condition)->update($update);
    }
    ...
    public function getOrderInfo($condition = array(), $extend = array(), $fields = '*', $order = '',$group = '') {
        ...
        if (in_array('order_goods',$extend)) {
            //取商品列表
            /*
             * [修改]     gongbo  20161208  判断goods_common表，从表中获取商品上下架状态
             */
            if(in_array('goods_common',$extend)){       //查询该订单商品下架状态
                $order_goods_list = $this->getOrderGoodsList_and_state(array('order_id'=>$order_info['order_id']));
            }else{          //正常查询
    ...
    /**
     * [添加]   获取商品上下架状态   Author:gongbo   Date:20161208
     * 取得订单商品表列表
     * @param unknown $condition
     * @param string $fields
     * @param string $limit
     ...
     * @param string $key
     */
    public function getOrderGoodsList_and_state($condition = array(), $fields = '*', $limit = null, $page = null, $order = 'rec_id desc', $group = null, $key = null) {

        $on = 'order_goods.goods_commonid = goods_common.goods_commonid';
        $fields = 'order_goods.*,goods_common.goods_state';
        return $this->table('order_goods,goods_common')->join('left')->on($on)->field($fields)->where($condition)->limit($limit)->order($order)->group($group)->key($key)->page($page)->select();
    }
    ...
    public function getOrderOperateState($operate,$order_info){
        ...
        switch ($operate) {
           ...
           case 'system_receive_pay':
               $state = $order_info['order_state'] == ORDER_STATE_NEW;
               $state = $state && $order_info['payment_code'] == ('online' || 'offbank') && $order_info['api_pay_time'];
               break;
