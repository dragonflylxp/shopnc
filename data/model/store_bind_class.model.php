...
class store_bind_classModel extends Model{
    ...
    public function getStoreList($condition){
        $on = "store.store_id = store_bind_class.store_id";
        $fields = '*';
        $list = $this->table('store_bind_class,store')->join('inner')->on($on)->where(array('bid'=>$condition['bid'],'store.store_id'=>$condition['store_id']))->field($fields)->select();
        return $list;
