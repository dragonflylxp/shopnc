...
class vr_orderModel extends Model {
    ...
    public function getOrderInfo($condition = array(), $fields = '*', $master = false,$lock = false) {
        ...
        /*
         * 虚拟订单产品 上下架状态  gongbo  20161208------start----------------
         *
         */
        $on = 'goods.goods_commonid = goods_common.goods_commonid';
        $fields = 'goods_common.goods_state';
        $cdn['goods_id'] = $order_info['goods_id'];
        $goods_common = Model()->table('goods,goods_common')->field($fields)->where($cdn)->join('left')->on($on)->find();
        $order_info['goods_state'] = $goods_common['goods_state'];
        //--------------------------------------------end-----------------------
        return $order_info;
    }
