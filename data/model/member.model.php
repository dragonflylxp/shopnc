...
class memberModel extends Model {
    ...
    public function register($register_info) {
        ...
        // 验证推荐码是否存在
        if(!empty($register_info['referral_code'])){
            $check_referral_code = $this->getMemberInfo(array('code'=>$register_info['referral_code']));
            if(is_array($check_referral_code) && count($check_referral_code)==0) {
                return array('error' => '推荐码无效');
            }
        }
        ...
        $member_info['referral_code']   = $register_info['referral_code'];
     ...
     * 联合注册/登录
     */
    public function comlogin($comlogin_info) {
        /*
        //重复操作
        if (process::islock('login')){
            return array('error' => '联合登录操作过于频繁，请稍后再试');
        }
        */
        // 参数验证
        $obj_validate = new Validate();
        $obj_validate->validateparam = array(
            array(
                "input" => $comlogin_info["agencyid"],
                "require" => "true",
                "message" => "请填写agencyid"
            ),
            array(
                "input" => $comlogin_info["merchantid"],
                "require" => "true",
                "message" => "请填写merchantid"
            ),
        );
        $error = $obj_validate->validate();
        if ($error != ''){
        ...
        // 验证agencyid/merchantid是否存在: 存在就登录，不存在就注册
        $member_info = $this->getMemberInfo(array('member_agencyid'=>$comlogin_info['agencyid'],
                                                  'member_merchantid'=>$comlogin_info['merchantid']));
        if(is_array($member_info) and count($member_info) > 0) {
            if(!$member_info['member_state']){
                return array('error' => '账号被停用');
            }
            ...
            // 会员添加
            $member_info    = array();
            $member_info['member_name']       = $comlogin_info['merchantid'];
            $member_info['member_passwd']     = $comlogin_info['merchantid'];
            $member_info['member_email']      = $comlogin_info['merchantid'];
            $member_info['member_agencyid']   = $comlogin_info['agencyid'];
            $member_info['member_merchantid'] = $comlogin_info['merchantid'];
            $insert_id  = $this->addMember($member_info);
            if($insert_id) {
                $member_info['member_id'] = $insert_id;
                $member_info['is_buy'] = 1;
    ...
    public function addMember($param) {
        ...
        try {
            ...
            $member_info['member_agencyid']     = $param['member_agencyid'];
            $member_info['member_merchantid']   = $param['member_merchantid'];
            ...
            $insert = $this->addPaymentSubAccount($sub_account);
            if (!$insert) {
                throw new Exception();
            }
            $insert = $this->addMemberGraph($insert_id,$param['member_name'],$param['referral_code']);
            if (!$insert) {
                throw new Exception();
            }
...
//            }
    ...
    public function addPaymentSubAccount($data) {
        $model = Model('sub_account');
        return $model->prefix('')->table('payment_sub_account')->insert($data);
    }

    public function addMemberGraph($member_id,$member_name,$referral_code) {
        $model_graph   = Model('member_graph');
        if(empty($referral_code)){
            $memberGraph = array();
            $memberGraph['member_id'] = $member_id;
            $memberGraph['member_name'] = $member_name;
            $memberGraph['graph_type'] = 1;
            $memberGraph['parent_id'] = 0;
            $memberGraph['layer'] = 1;
            $memberGraph['referral_id'] = 0;
            $member_graph_id = $model_graph->prefix('')->table('secs_member_graph')->insert($memberGraph);
            $model_graph->prefix('')->table('secs_member_graph')->where(array('id'=>$member_graph_id))->update(array('path'=>strval($member_graph_id)));
            return $member_graph_id;
        }else{
            $top_member = $this->table('member')->field('member_id')->where(array('code' => $referral_code))->master(false)->find();
            $top_member_graph = $model_graph->prefix('')->table('secs_member_graph')->field('*')->where(array('member_id'=>$top_member['member_id']))->master(false)->find();
            $memberGraph = array();
            $memberGraph['member_id'] = $member_id;
            $memberGraph['member_name'] = $member_name;
            $memberGraph['graph_type'] = 1;
            $memberGraph['parent_id'] = $top_member_graph['id'];
            $memberGraph['layer'] = $top_member_graph['layer']+1;
            $memberGraph['referral_id'] = $top_member_graph['member_id'];
            $member_graph_id = $model_graph->prefix('')->table('secs_member_graph')->insert($memberGraph);
            $model_graph->prefix('')->table('secs_member_graph')->where(array('id'=>$member_graph_id))->update(array('path'=>$top_member_graph['path'].'-'.strval($member_graph_id)));
            return $member_graph_id;
        }
    }
     ...
     * @param $condition
     */
    public function get_bill_bank_infoOp($condition,$field = ''){
     ...
     * 获取手机号下面的多个账号
     * */
    public function getMemberAllInfo($condition, $field = '*', $master = false) {
        return $this->table('member')->field($field)->where($condition)->master($master)->select();
    }
	/**
	 * 获取邀请人的信息 20160906
	 */
