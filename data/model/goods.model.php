...
class goodsModel extends Model{
    ...
    public function getGoodsList($condition, $field = '*', $group = '',$order = '', $limit = 0, $page = 0, $count = 0,$extend='') {    //gongbo 添加$extend参数  20161226  评论销量共享
        ...
        if($extend == 'goods_common' && !empty($extend)){
            $on = 'goods.goods_commonid = goods_common.goods_commonid';
            $field = $field.',goods_common.sale_count';
            $rs =  $this->table('goods,goods_common')->field($field)->join('left')->on($on)->where($condition)->group($group)->order($order)->limit($limit)->page($page, $count)->select();
            foreach($rs as $k=>$v){
                $rs[$k]['goods_salenum'] = $v['sale_count'];
            }
            return $rs;
        }else{
     ...
     * @return array
     */
    public function getGoodsListByColorDistinct($condition, $field = '*', $order = 'goods.goods_id asc', $page = 0, $limit = 0,$extend='') {      //gongbo 20161226  添加extend参数
        $condition['goods.goods_state']   = self::STATE1;
        $condition['goods.goods_verify']  = self::VERIFY1;
        $condition = $this->_getRecursiveClass($condition);
        if (C('dbdriver') == 'mysql') {
            $_field = "CONCAT(goods.goods_commonid,',',goods.color_id)";
        ...
        $goods_list = array();
        if (C('dbdriver') == 'mysql') {
            $goods_list = $this->getGoodsOnlineList($condition, $_field.' nc_distinct,'.$field, $page, $order, $limit, $_distinct, false, $count,$extend);
        } elseif (C('dbdriver') == 'oracle') {
     ...
     * @return array
     */
    public function getGoodsOnlineList($condition, $field = '*', $page = 0, $order = 'goods_id desc', $limit = 0, $group = '', $lock = false, $count = 0,$extend='') {  //gongbo 添加extend参数20161226
        $condition['goods.goods_state']   = self::STATE1;
        $condition['goods.goods_verify']  = self::VERIFY1;
        return $this->getGoodsList($condition, $field, $group, $order, $limit, $page, $count,$extend);
    }
