...
class evaluate_goodsModel extends Model {
     ...
     * gongbo 20161213
     */
    public function getEvaluateGoodsList_b($condition, $page = null, $limit = null, $order = 'geval_id desc', $field = '*') {
        //先找出该商品commonid
        $db =  Model('goods');
        $where['goods_id'] = $condition['geval_goodsid'];
        $goodsRs = $db->getGoodsInfo($where,$field='goods_commonid,store_id');          //20161226
        //echo $goodsRs = $db->where($where)->field('goods_commonid,store_id')->find();
        //$goodsList = $db -> where(array('goods_commonid'=>$goodsRs['goods_commonid'],'store_id'=>$goodsRs['store_id']))->find();
        $on = 'goods.goods_id=evaluate_goods.geval_goodsid';
        $condition['goods.goods_commonid']  = $goodsRs['goods_commonid'];
        $condition['goods.store_id']        = $goodsRs['store_id'];
        unset($condition['geval_goodsid']);
        $msgList = Model()->table('goods,evaluate_goods')->join('inner')->on($on)->where($condition)->field('evaluate_goods.*')->page($page)->limit($limit)->order($order)->select();
        //$list = $this->field($field)->where($condition)->page($page)->limit($limit)->order($order)->select();
        return $msgList;
    }
    ...
    public function getEvaluateGoodsInfoByGoodsID($goods_id) {
        ...
        if(empty($info)) {
            /*
             * sku商品评论通用  1，获取goods_commonid-------------start----------------
             */
            $db =  Model('goods');
            $where['goods.goods_id']=$goods_id;
            $goodsRs = $db->where($where)->field('goods_commonid,store_id')->find();
            if($goodsRs){
                //查询commonid相同的商品
                $info = $c_array = array();
                $count_array = array();
                $goods_id_list = $db -> where(array('goods_commonid'=>$goodsRs['goods_commonid'],'store_id'=>$goodsRs['store_id']))->field('goods_id')->select();
                $i = 0;
                foreach($goods_id_list as $v){
                    $arr = $this->field('count(*) as count,geval_scores')->where(array('geval_goodsid' => $v['goods_id']))->group('geval_scores')->key(geval_scores)->select();
                    $c_array = array_merge($arr,$c_array);
                }

                $count_1 = $count_2 = $count_3 =$count_4 = $count_5 =0;
                for($i=0;$i<count($c_array);$i++){
                    switch($c_array[$i]['geval_scores']){
                        case 1:
                            $count_1 += $c_array[$i]['count'];
                            break;
                        case 2:
                            $count_2 += $c_array[$i]['count'];
                            break;
                        case 3:
                            $count_3 += $c_array[$i]['count'];
                            break;
                        case 4:
                            $count_4 += $c_array[$i]['count'];
                            break;
                        case 5:
                            $count_5 += $c_array[$i]['count'];
                            break;
                    }
                }
            }
            ...
            $star1 = intval($count_1);
            $star2 = intval($count_2);
            $star3 = intval($count_3);
            $star4 = intval($count_4);
            $star5 = intval($count_5);
            /*
             * sku商品评论通用  1，获取goods_commonid-------------end----------------
             */
