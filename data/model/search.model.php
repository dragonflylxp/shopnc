...
class searchModel{
    ...
    private $_indexer_name = array();
    //全文搜索结果所花时间
    private $_indexer_time;
    //全文搜索结果相关搜索
    private $_indexer_related = array();
    //全文搜索热门搜索
    private $_indexer_hot = array();
	//全文搜索你是不是想找
    private $_indexer_corrected = array();	
	private $_index_corrected = array();
    //全文搜索总库量
    private $_indexer_total;
    ...
    public function getIndexerList($condition = array(), $order = array(), $pagesize = 24) {
        try {
            ...
            //执行搜索
            $this->_searchXS();
            return array($this->_goods_list, $this->_indexer_count,$this->_indexer_ids,$this->_indexer_name,$this->_indexer_time,$this->_indexer_related,$this->_indexer_hot,$this->_indexer_corrected,$this->_indexer_total,$this->_index_corrected);
...
//                 showMessage($e->getMessage(),'','html','error');
                ...
                shopec\Log::record('search\index'.$e->getMessage()."\r\n",shopec\Log::RUN);
                return false;
            }
        }
    }
    ...
    private function _createXS($pagesize,$appname) {
        ...
        $this->_xs_search->setAutoSynonyms(); //20170118
        //设置分面
        if ($this->_open_face) {
    ...
    private function _searchXS(){
        ...
        foreach ($docs as $doc) {
			...
			$goods_name[] = $this->_xs_search->highlight(htmlspecialchars($doc->goods_name));
		...
		//得到其他搜索结果
		$hot = $this->_xs_search->getHotQuery();
		$total = $this->_xs_search->getDbTotal();
			// try to corrected, if resul too few
			if ($count < 1 || $count < ceil(0.001 * $total)) {
				$corrected = $this->_xs_search->getCorrectedQuery($this->_indexer_xs);
			}
			// get related query
			$related = $this->_xs_search->getRelatedQuery($this->_indexer_xs);
		$search_cost = microtime(true) - $search_begin;
		$this->_indexer_name = $goods_name;
		$this->_indexer_total = $total;
		$this->_indexer_time = $search_cost;
		$this->_indexer_related = $related;
		$this->_indexer_hot = $hot;
		$this->_indexer_corrected = $corrected;        
        return true;
    }
   ...
   public function getTagCategory($keyword = '') {
    ...
    public function indexerSearch($get = array(), $pagesize) {
        ...
        if ($result !== false) {
            list($goods_list, $indexer_count,$indexer_ids,$indexer_name,$indexer_time,$indexer_related,$indexer_hot,$indexer_corrected,$indexer_total,$index_corrected) = $result;
			...
			$indexer_name = null;
			$indexer_time = 0;
			$indexer_related = null;
			$indexer_hot = null;
			$indexer_corrected = null;
			$indexer_total = 0;
        }
 		if($cond == 'UNEXPAND'){
			 return array($goods_list, $indexer_count,$indexer_ids,$indexer_name,$indexer_time,$indexer_related,$indexer_hot,$indexer_corrected,$indexer_total,$index_corrected);
		}else{		
		    if (empty($indexer_corrected)) {
                return array($goods_list, $indexer_count,$indexer_ids,$indexer_name,$indexer_time,$indexer_related,$indexer_hot,$indexer_corrected,$indexer_total,$index_corrected);
            }else{
		        $condition['keyword']=$index_corrected=$indexer_corrected[0];
		        $this->_index_corrected = $index_corrected;
                //取得商品主键等信息
                $result = $this->getIndexerList($condition,$order,$pagesize);
                if ($result !== false) {
                    list($goods_list, $indexer_count,$indexer_ids,$indexer_name,$indexer_time,$indexer_related,$indexer_hot,$indexer_corrected,$indexer_total,$index_corrected) = $result;
    ...
    public function autoCompletewap($get) {
    ...
    public function autoComplete($get) {
        ...
        try {
            ...
            if (count($corrected) !== 0) {
                $data = array();
                foreach ($corrected as $kc=>$wordv){     
                		        	
                	if(stripos($wordv,$get['term']) !==false ){
                		          
                		$row['em'] = $get['term'];
                		$row['keyword'] = substr($wordv,$wordk,strlen($wordv));
                		$row['setword'] = $wordv;
                		if($aps > 3){
                		    $aac = $this->setFacetsa($wordv,$obj_search,$kc);
                		    if($aac){
                			    $fen[] = $aac;
                		    }
                		}	
     ...
     * 创建全文搜索对象，分面搜索
     */
    private function setFacetsa($facetsa_name,$search_dx,$kc) {  	
    	$aaa = $search_dx->setQuery($facetsa_name)->setFacets(array('gc_id'),true)->search();
    	// 读取分面结果[id][个数]
		$fid_counts = $search_dx->getFacets('gc_id'); 
		//$ccc_counts = $search_dx->getFacets('gc_name');
		$gc_model = Model('goods_class');
		if(!empty($fid_counts)){
			$last_id =array();
			
			foreach($fid_counts as $kk=>$vv){
				//分面商品个数
				if($vv > '300'){
					//获取分类信息				
					$last_id['type']='hot';
				...
				if($vv > '50'){
					$last_id['id']= $kk;					
					$cate = $gc_model->getGoodsClassNav($kk);
					$last = array_pop($cate);
					$last_id['title']= mb_substr($last['title'],0,4,"utf-8");
					$last_id['number']= $vv;
					$dare[] = $last_id;			
				}
