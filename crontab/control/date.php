...
class dateControl extends BaseCronControl {
    ...
    private function _goods_click_update() {
        ...
        foreach ($data as $key => $val) {
            ...
            $goods = Model('goods')->getGoodsInfoByID($key);
            Model($goods)->editGoodsCommonById(array('click_count' => array('exp', 'click_count +'.$val)),$goods['goods_commonid']);
        }
    ...
    private function _create_bill_cycle_by_month($start_unixtime,$store_info) {
        ...
        //结算周期为8天默认时间----------by suijiaolong---start-------
        $i = 7;
    ...
    private function _calc_real_order_bill($data_bill){
        ...
        /*//----------by gongbo---start-------
        if($this->_model_bill->editOrderBill($update,array('ob_id'=>$data_bill['ob_id']))){
            ...
            if($result_amount>0){
                ...
                return $this->_create_member_real_order_bill($m_data_bill);           //生成会员虚拟结算记录
            }else{
                return true;
            }
        }else{
    ...
    private function _calc_vr_order_bill($data_bill){
        ...
        /*//----------by gongbo---start-------
        if($this->_model_vr_bill->editOrderBill($update,array('ob_id'=>$data_bill['ob_id']))){
    ...
    private function _get_refer_member_id($store_id){
        ...
        if($result && !empty($result['r_code'])){        //有推荐人
            $memberRs = Model('member')->where(array('code'=>$result['r_code']))->field('member_id')->find();
            if($memberRs){      //该推荐人存在
