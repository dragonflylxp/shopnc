...
function weixin_login(){
    ...
    $.getScript("http://res.wx.qq.com/connect/zh_CN/htmledition/js/wxLogin.js", function(){
        var obj = new WxLogin({
            ...
            scope: "snsapi_login", 
            redirect_uri: MEMBER_SITE_URL+"/index.php?con=connect_wx&fun=get_info",
            state: "",
            style: "",
            href: ""
        });
...
(function($) {
    $.show_nc_login = function(options) {
        ...
        if (connect_qq == '1'){
            add_html += '<a href="'+MEMBER_SITE_URL+'/index.php?con=connect_qq" title="QQ账号登录" class="qq"><i></i></a>';
        }
        if (connect_sn == '1'){
            add_html += '<a href="'+MEMBER_SITE_URL+'/index.php?con=connect_sina" title="新浪微博账号登录" class="sina"><i></i></a>';
        }
        if (connect_wx == '1'){
        ...
        login_dialog_html.append('<form class="bg" method="post" id="ajax_login" action="'+settings.action+'"></form>').find('form')
        	...
        	.append('<ul><li>›&nbsp;如果您没有登录账号，请先<a class="register" href="'+LOGIN_SITE_URL+'/index.php?con=login&amp;op=register">注册会员</a>然后登录</li><li>›&nbsp;如果您<a class="forget" href="'+LOGIN_SITE_URL+'/index.php?con=login&amp;op=forget_password">忘记密码</a>？，申请找回密码</li></ul>')
...
(function($){
	$.fn.membershipCard = function(options){
		                ...
		                success: function(data) {
		                	if(data){
		                		...
		                		if(data.follow != 2){
		                			...
		                			$('<div class="send-msg"> <a href="'+MEMBER_SITE_URL+'/index.php?con=member_message&fun=sendmsg&member_id='+data.id+'" target="_blank"><i></i>站内信</a> </div>').appendTo(_bottom);
		                		}
		...
		function follow(o){
			...
			$.getJSON(MEMBER_SITE_URL+'/index.php?con=member_snsfriend&fun=addfollow&callback=?&mid='+data_str.mid, function(data){
				if(data){
		...
		function onfollow(o){
			var data_str = o.parent().attr('data-param');
			eval( "data_str = "+data_str);
			$.getJSON(MEMBER_SITE_URL+'/index.php?con=member_snsfriend&fun=delfollow&callback=?&mid='+data_str.mid, function(data){
				if(data){
