...
$(function(){
   ...
   if (agencyId != null && agencyId != "" && merchantId != null && merchantId != "") {
       comlog(agencyId, merchantId, order_list);
   }
   else{
       order_list();
   }
...
function order_list(){
	...
	if(!key && parseInt(agencyid) <= 0 && parseInt(merchantid) <= 0){
	...
	function initPage(){
		...
		$.ajax({
			type:'post',
			url:ApiUrl+"/index.php?con=member_order&fun=order_list&page="+page+"&curpage="+curpage,
