...
$(function(){
   ...
   if (agencyId != null && agencyId != "" && merchantId != null && merchantId != "") {
       comlog(agencyId, merchantId, product_list);
   }
   else{
       product_list();
   }
...
function product_list(){
    ...
    $('#product_list').on('click', '.goods-store a',function(){
        ...
        $.getJSON(ApiUrl + '/index.php?con=store&fun=store_credit', {store_id:store_id}, function(result){
            var html = '<dl>'
...
function get_list() {
    ...
    $.getJSON(ApiUrl + '/index.php?con=goods&fun=goods_list' + window.location.search.replace('?','&'), param, function(result){
    	if(!result) {
    		result = [];
    		result.datas = [];
...
function search_adv() {
    $.getJSON(ApiUrl + '/index.php?con=index&fun=search_adv', function(result) {
