 ...
 //图片上传
    $("#article_image_upload").fileupload({
        dataType: 'json',
            url: "index.php?con=publish&fun=article_image_upload",
            add: function(e,data) {
    ...
    $("[nctype='btn_drop_article_image']").live("click",function(){
        ...
        $.getJSON("index.php?con=publish&fun=article_image_drop", { image_name: image_name }, function(result){
            if(result.status == "success") {
                image_object.remove();
            } else {
                showError(result.error);
        ...
        if($("#article_goods_list li").length < 3) { 
            ...
            if(search_keyword != "") {
                if(search_type == "goods_url") {
                    ...
                    $.getJSON("index.php?con=api&fun=goods_info_by_url", { url: url}, function(data){
                        if(data.result == "true") {
                    ...
                    $("#div_goods_select").load("index.php?con=api&fun=goods_list&search_type="+search_type+"&search_keyword="+search_keyword);
                }
            }
        }
    });
    ...
    $("#btn_article_search").click(function(){
        ...
        if(search_keyword != "") {
            $("#div_article_select").load("index.php?con=api&fun=article_list&search_type="+search_type+"&search_keyword="+search_keyword);
        }
    });
