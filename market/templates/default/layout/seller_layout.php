...
<header class="ncsc-head-layout w">
  <div class="wrapper">
    ...
    <div class="center-logo"> <a href="<?php echo SHOP_SITE_URL;?>/index.php?con=seller_center&fun=index" target="_blank"><img src="<?php echo UPLOAD_SITE_URL.'/'.ATTACH_COMMON.DS.C('seller_center_logo');?>" class="pngFix" alt=""/></a>
      <h1>商家中心</h1>
    </div>
        ...
        <div class="sitemap-menu">
          ...
          <div id="quicklink_list" class="content">
            ...
            <dl <?php echo ($key == 'promotion' ? 'class="double"' : '');?>>
              ...
              <dd <?php if(!empty($output['seller_quicklink'])) {echo in_array($submenu_value['con'], $output['seller_quicklink'])?'class="selected"':'';}?>>
                <a href="<?php echo $key != 'distribute' ? SHOP_SITE_URL.'/':'';?>index.php?con=<?php echo $submenu_value['con'];?>&fun=<?php echo $submenu_value['fun'];?>"> <?php echo $submenu_value['name'];?> </a></dd>
              <?php } ?>
              <?php } ?>
            </dl>
            <?php } ?>
    ...
    <nav class="ncsc-nav">
      <dl class="<?php echo $output['current_menu']['model'] == 'index'?'current':'';?>">
        <dt><a href="<?php echo SHOP_SITE_URL;?>/index.php?con=seller_center&fun=index">首页</a></dt>
        <dd class="arrow"></dd>
      </dl>
      <?php if(!empty($output['menu']) && is_array($output['menu'])) {?>
      <?php foreach($output['menu'] as $key => $menu_value) {?>
      <dl class="<?php echo $output['current_menu']['model'] == $key?'current':'';?>">
        <dt><a href="<?php echo $key != 'distribute' ? SHOP_SITE_URL.'/':'';?>index.php?con=<?php echo $menu_value['child'][key($menu_value['child'])]['con'];?>&fun=<?php echo $menu_value['child'][key($menu_value['child'])]['fun'];?>"><?php echo $menu_value['name'];?></a></dt>
        <dd>
          <ul>
            <?php if(!empty($menu_value['child']) && is_array($menu_value['child'])) {?>
            <?php foreach($menu_value['child'] as $submenu_value) {?>
            <li> <a href="<?php echo $key != 'distribute' ? SHOP_SITE_URL.'/':'';?>index.php?con=<?php echo $submenu_value['con'];?>&fun=<?php echo $submenu_value['fun'];?>"> <?php echo $submenu_value['name'];?> </a> </li>
            <?php } ?>
            <?php } ?>
          </ul>
        </dd>
