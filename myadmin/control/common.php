...
//      $image = \shopec\Lib::imager()->createImageFromPath($save_file);
...
//      Tpl::output('width', $image->width);
    ...
    private function get_pending_matters() {
        ...
        //消费者保障服务申请
        if($this->checkPermissionPending('contract','shop')){
            $statistics['contract'] =   Model('contract')->getContractCount();
            $statistics['contract_quit'] =   Model('contract')->getContractQuitCount();
        }

        //银行汇款订单
        if($this->checkPermissionPending('order','shop')){
            $statistics['order_offbank'] =  $model_order->getPayOffbankCount();
            $statistics['order_vr_offbank'] =  $model_order->getVrPayOffbankCount();
        }

        if($this->checkPermissionPending('store','shop')){
            // 预存款提现
        ...
        //管理人实物提现申请
        if($this->checkPermissionPending('manager_rebill_check','shop')){
            $statistics['get_money_apply'] =  Model('manager_real_bill')->getMoneyApplyCount();
        }
        //管理人虚拟提现申请
        if($this->checkPermissionPending('manager_vrbill_check','shop')){
            $statistics['get_vrmoney_apply'] =  Model('manager_vr_bill')->getMoneyApplyCount();
        }

        //管理人申请
        if($this->checkPermissionPending('manager','system')){
            $statistics['manager_apply'] =  Model('manager_member')->getManagerApplyCount();
        }
        ...
        // 举报
        if($this->checkPermissionPending('','shop')){
            $statistics['inform'] = Model('inform')->getInformCount(array('inform_state'=>1));
        }
        // 品牌申请
        if($this->checkPermissionPending('brand','shop')){
        ...
        // 团购数量
        if($this->checkPermissionPending('groupbuy','shop')){
        ...
        if($this->checkPermissionPending('bill','shop')){
            $statistics['check_billno'] = $model_bill->getOrderBillCount($condition);
        }
        if($this->checkPermissionPending('vr_bill','shop')){
        ...
        // 平台客服
        if($this->checkPermissionPending('mall_consult','shop')) {
         ...
         * 资讯
         */
        if($this->checkPermissionPending('','cms')){
            if (C('cms_isuse')) {
                // 文章审核
         ...
         * 圈子
         */
        if($this->checkPermissionPending('','circle')){
            if (C('circle_isuse')) {
         ...
         * 分销
         */
        if($this->checkPermissionPending('','distribute')){
