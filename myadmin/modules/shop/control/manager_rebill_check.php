...
class manager_rebill_checkControl extends SystemControl{
    ...
    private $_links = array(
        array('url'=>'con=manager_rebill_check&fun=index','text'=>'待审核申请'),
        array('url'=>'con=manager_rebill_check&fun=get_money_state','text'=>'提现状态')
    );
    ...
    public function indexOp()
    {
        ...
        Tpl::showpage('manager_rebill_check.index');
    }
    ...
    public function index_xmlOp()
    {
        ...
        foreach ($list as $val) {
            ...
            if(in_array(intval($val['state']), array(2))) {
                $operation = "<a class='btn orange' href=\"index.php?con=manager_rebill_check&fun=show_manager_real_bill&member_id=". $val['uid'] ."\"><i class=\"fa fa-check-circle\"></i>审核</a>";
            } else {
                $operation = "<a class='btn green' href=\"index.php?con=manager_rebill_check&fun=show_manager_real_bill&member_id=". $val['uid'] ."\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            }
     ...
     *
     */
    public function show_manager_real_billOp(){
        ...
        $model_manager_real_bill = Model('manager_real_bill');
        $manager_real_bill_info = $model_manager_real_bill->getManagerRealBillInfo(array('member_id'=>$member_id));
        ...
        Tpl::showpage('manager_rebill_check.show');
    }
    ...
    public function bill_check_verifyOp() {
        $model_manager = Model('manager_real_bill');
//        $manager_state = $model_manager->getOne(array('manager_id'=>$_POST['manager_id']));
        ...
        if($verify_type == 3){
            $param['state'] = 3;
            $this->log("管理人ID为 :".$member_id." 的地区管理人实物提现申请审核通过");
        }
        if($verify_type == 5){
            $param['state'] = 5;
            $param['pay_content'] = $_POST['pay_content'];
            $this->log("管理人ID为 :".$member_id." 的地区管理人实物提现申请审核失败");
        }

        $result = $model_manager->editManagerInfo(array('uid' => $member_id), $param);
        if ($result) {
            showMessage('提交成功', 'index.php?con=manager_rebill_check&fun=index');
        } else {
            showMessage('提交失败');
        }
    ...
    public function get_money_stateOp()
    {
        ...
        Tpl::showpage('manager_rebill_state.index');
    }
    /**
     * 显示管理人实物结算列表审核
     * 申请状态，1默认未申请，2申请，3审核完成，4确认打款，5审核未通过
    ...
    public function get_index_xmlOp()
    {
        ...
        foreach ($list as $val) {
            ...
            if(in_array(intval($val['state']), array(3))) {
                $operation = "<a class='btn orange' href=\"index.php?con=manager_rebill_check&fun=show_manager_state&member_id=". $val['uid'] ."\"><i class=\"fa fa-check-circle\"></i>确认提现</a>";
            } else {
                $operation = "<a class='btn green' href=\"index.php?con=manager_rebill_check&fun=show_manager_state&member_id=". $val['uid'] ."\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            }
            $i['operation'] = $operation;
    ...
    public function show_manager_stateOp(){
        ...
        Tpl::output('bill_info',$manager_real_bill_info);
        Tpl::showpage('manager_rebill_state.show');
    }
    /**
     * 管理人提现确认
     */
...
//        $manager_state = $model_manager->getOne(array('manager_id'=>$_POST['manager_id']));
        ...
        if($verify_type == 4){
            ...
            $this->log("管理人ID为 :".$member_id." 的地区管理人实物结算已提现");
        }
        if($verify_type == 3){
            $param['state'] = 3;
        }
        ...
        $result = $model_manager->editManagerInfo(array('uid' => $member_id), $param);
        if ($result) {
            showMessage('提交成功', 'index.php?con=manager_rebill_check&fun=get_money_state');
        } else {
            showMessage('提交失败');
        }
    ...
    public function export_step1Op()
    {
        ...
        if (!is_numeric($_GET['curpage'])) {
            ...
            if ($count > self::EXPORT_SIZE) {
                ...
                Tpl::output('murl', 'index.php?con=manager_rebill_check&fun=index');
...
//            var_dump($data);exit;
    ...
    public function del_cardOp()
    {
        ...
        $count = count($id);
        Model('manager_real_bill')->delManagerById($id);
