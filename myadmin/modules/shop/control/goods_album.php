...
class goods_albumControl extends SystemControl{
    ...
    public function get_xmlOp() {
        ...
        foreach ($album_list as $value) {
            $param = array();
            $operation = "<a class='btn green' href='index.php?con=goods_album&fun=pic_list&aclass_id=".$value['aclass_id']."'><i class='fa fa-list-alt'></i>查看</a>";//产品库用的默认相册不能删除
            if ($value['store_id']) $operation = "<a class='btn red' href='javascript:void(0);' onclick='fg_del(". $value['aclass_id'] .")'><i class='fa fa-trash-o'></i>删除</a><a class='btn green' href='index.php?con=goods_album&fun=pic_list&aclass_id=".$value['aclass_id']."'><i class='fa fa-list-alt'></i>查看</a>";
