...
class billControl extends SystemControl{
    ...
    public function show_billOp(){
        ...
        /*
         * 显示结算帐号的信息   gongbo 20161213  ----------------start---------------
         */
        $model_bd = Model();
        $field = 'store_joinin.settlement_bank_account_name,store_joinin.settlement_bank_account_number,store_joinin.settlement_bank_name,store_joinin.settlement_bank_code,store_joinin.settlement_bank_address,store.is_own_shop';
        $on = 'store.member_id=store_joinin.member_id';
        $model_bd->table('store,store_joinin')->field($field);
        $bank_rs = $model_bd->join('left')->on($on)->where(array('store.store_id'=>$bill_info['ob_store_id']))->find();
        if($bank_rs['is_own_shop'] ==1 && $bill_info['ob_store_id'] == 60){    //为自营店
            $bank_rs['settlement_bank_account_name'] = '神农币商业管理(大连)有限公司';
            $bank_rs['settlement_bank_account_number'] = '21250186005000000094';
            $bank_rs['settlement_bank_name'] = '中国建设银行股份有限公司大连青泥洼桥支行';
            $bank_rs['settlement_bank_code'] = '105222086504';
            $bank_rs['settlement_bank_address'] = '大连';
        }

        Tpl::output('bill_bank_info',$bank_rs);
        /*
        * 显示结算帐号的信息   gongbo 20161213  ----------------end---------------
        */
    ...
    public function export_billOp(){
        ...
        foreach ($data as $k => $v) {
            ...
            $ob_dis_pay_amount += $export_data[$k+1][] = $v['ob_dis_pay_amount'];
    ...
    public function export_orderOp(){
        ...
        $export_data[0] = array('订单编号','支付单号','支付方式','支付时间','订单金额','运费','佣金','下单日期','成交日期','商家','商家编号','买家','买家编号','商品');
