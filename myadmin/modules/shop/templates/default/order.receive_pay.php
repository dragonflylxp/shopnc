  ...
  <form method="post" name="form1" id="form1" action="index.php?con=<?php echo $_GET['con'];?>&fun=change_state&state_type=receive_pay&order_id=<?php echo intval($_GET['order_id']);?>">
    ...
    <div class="ncap-form-default">
     ...
     <?php if($output['order_info']['payment_code']!='offbank'){ ?>
      ...
      <?php }else{ ?>
        <dd class="opt">
		  <input type='hidden' name='payment_code' value='offbank'/>
		  <span>银行汇款</span>
          <span class="err"></span>
          <p class="notic"></p>
        </dd>
      ...
      <dl class="row">
        <dt class="tit">
          <label for="closed_reason">银行汇款单号</label>
        </dt>
        <dd class="opt">
          <input type="text" class="txt2" name="trade_no" id="trade_no" maxlength="40" value="<?php echo $output['order_info']['trade_no'];?>">
          <span class="err"></span>
          <p class="notic"><span class="vatop rowform">银行汇款单号</span></p>
        </dd>
      </dl>
      <?php }?>
        <dl class="row">
