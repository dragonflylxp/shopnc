...
class personal_classControl extends SystemControl{
    ...
    const MICROSHOP_CLASS_LIST = 'index.php?con=goods_class&fun=goodsclass_list';
    const GOODS_FLAG = 1;
    ...
    public function personalclass_saveOp() {
        ...
        if($result) {
            showMessage(Language::get('class_add_success'),"index.php?con=personal_class&fun=personalclass_list");
        } else {
            showMessage(Language::get('class_add_fail'),"index.php?con=personal_class&fun=personalclass_list",'','error');
        }
     ...
     **/
     ...
     private function show_menu_personal_class($menu_key) {
         $menu_array = array(
                 'personal_class_list'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_manage'),'menu_url'=>'index.php?con=personal_class&fun=personalclass_list'),
                 'personal_class_add'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_new'),'menu_url'=>'index.php?con=personal_class&fun=personalclass_add'),
         );
