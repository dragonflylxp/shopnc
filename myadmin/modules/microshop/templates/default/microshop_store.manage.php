...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?con=store&fun=store_manage_xml',
        colModel: [
        ...
        buttons: [
            {
                ...
                onpress: function() {
                    ...
                    location.href = 'index.php?con=store&fun=store_drop_save&store_id=__IDS__'.replace('__IDS__', ids.join(','));
                }
            }
        ],
        searchitems: [
...
$('a[data-ie-column]').live('click', function() {
    $.get('index.php?con=store&fun=ajax&branch=store_commend', {
...
$("span[data-live-inline-edit='microshop_sort']").live('click', function() {
    ...
    $input.change(function() {
        ...
        $.getJSON('index.php?con=store&fun=store_sort_update', {
            id: $this.parents('tr').attr('data-id'),
            value: v2
        }, function(d) {
            if (d.result) {
