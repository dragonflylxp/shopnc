...
<div class="page">
  ...
  <form method="post" enctype="multipart/form-data" name="form1">
    ...
    <div class="ncap-form-default">
      ...
      <dl class="row">
        <dt class="tit">
          <label for="default_area"> <?php echo '默认城市';?></label>
        </dt>
        <dd class="opt">
        	<?php $areaa = unserialize($output['list_setting']['default_area']);?>
          <select id="default_area" name="default_area">
          	<?php if(!empty($output['area'])){?>
          	<?php foreach($output['area'] as $area){?>          	
                      	<option value="<?php echo $area['area_id'];?>" <?php if($area['area_id'] == $areaa['area_id']){ echo 'selected';}?>><?php echo $area['area_name'];?></option>
            <?php }?>
          	<?php }?>      
          </select>
          <p class="notic"><?php echo '选择默认城市，定位失败为默认';?></p>
        </dd>
      </dl>        
      <dl class="row">
        <dt class="tit">
