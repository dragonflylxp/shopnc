...
<div class="page">
  ...
  <form id="manager_form" method="post">
    ...
    <input type="hidden" name="mid" value="<?php echo $output['manager_info']['mid'];?>" />
    <div class="ncap-form-default">
      <dl class="row">
        ...
        <dd class="opt">
       	 	<p> <?php echo managerGrade($output['manager_info']['grade']);?></p>
            <select name="grade" id="grade" class="input-txt" onchange="set_area(this);" style="display: none;" >
          ...
          <span class="err"></span>
          <p class="notic" style="display:none;">选择管理人所属级别</p>
        </dd>
      </dl>
      <dl class="row">
          <dt class="tit">
          ...
          <dd class="opt" >
          <p> <?php echo $output['manager_info']['region']?></p>
            <select name="area" id="area"  class="input-txt" onchange="get_province(this);"  disabled='true' style="display: none;" ">
          ...
          <span class="err"></span>
          <p class="notic" style="display:none;">管理人绑定地区，请选择级别后再选择地区</p>
          </dd>
      </dl>
      <dl class="row">
        <dt class="tit">
        ...
        <dd class="opt">
            ...
            <?php foreach ($output['manager_list'] as $km=>$vm){ ?>
                <option value="<?php echo $vm['member_id'];?>" <?php if($output['manager_info']['uid']==$vm['member_id']) echo "selected";?>><?php echo $vm['company_name'];?></option>
            <?php }?>
            </select>
        ...
        <dd class="opt">
          <input type="text" value="<?php echo $output['manager_info']['point'];?>" name="point" id="point" class="input-txt">%
          <span class="err"></span>
...
<script>
	$(function(){
		$("#submitBtn").click(function(){
			if(validate()){
			    if(!confirm('提示：如果此时修改管理人信息，本月该地区所有订单提成都将属于该管理人，确认修改吗？')){
			        return false;
			    }
				IsBindArea();
			}
	...
	function IsBindArea(){
		var uid = $("#uid").val();
		var manager_id = <?php echo $output['manager_info']['uid'];?>;
		if(uid == manager_id){
			$("#manager_form").submit();
			return;
		}
		$.getJSON('index.php?con=manager_member&fun=ajaxIsBindArea&uid='+uid, function(result){
			if(result){
				if(!confirm('提示：该管理人已绑定在其他区域，是否继续？')){
					return false;
				}
				$("#manager_form").submit();
			}else{
