...
class mb_specialControl extends SystemControl{
     ...
     * 海报列表
     */
    public function qrcode_listOp()
    {
        $model_qrcode = Model('qrcode_tmpl');

        $mb_qrcode = $model_qrcode->select();
        foreach ($mb_qrcode as $key => $val) {
            $mb_qrcode[$key]['img_url'] = getWxShareImage($val['image']);
        }

        Tpl::output('list', $mb_qrcode);

        $this->show_menu('qrcode_list');

        Tpl::showpage('mb_qrcode.list');
    }
    /**
     * 图片上传
     */
    public function share_image_uploadOp()
    {
        $data = array();
        if (!empty($_FILES['share_image']['name'])) {
            $upload = new UploadFile();
            $upload->set('default_dir', ATTACH_MOBILE . DS . 'home' . DS);
            $upload->set('fprefix', '');
            $upload->set('allow_type', array('gif', 'jpg', 'jpeg', 'png'));
            $result = $upload->upfile('share_image');
            if (!$result) {
                $data['error'] = $upload->error;
            }
            $data['image_name'] = $upload->file_name;
            $data['image_url'] = getWxShareImage($data['image_name']);
        }
        echo json_encode($data);
    }
    public function qrcode_saveOp()
    {
        $model_share = Model('qrcode_tmpl');

        $param = array();
        $param['title'] = $_POST['title'];
        $param['type'] = '1';
        $result = $model_share->insert($param);

        if ($result) {
            //showMessage(L('nc_common_save_succ'), urlAdminMobile('mb_special', 'special_list'));
            echo json_encode(array('datas' => '保存成功'));
        } else {
            echo json_encode(array('datas' => '保存失败'));
            //showMessage(L('nc_common_save_fail'), urlAdminMobile('mb_special', 'special_list'));
        }
    }

    public function qrcode_updateOp()
    {
        $model_share = Model('qrcode_tmpl');
        ...
        $param['image'] = $_POST['image'];
        $param['position'] = htmlspecialchars_decode($_POST['position']);
        $result = $model_share->where(array('id' => $_POST['id']))->update($param);
    ...
    private function show_menu($menu_key='') {
        ...
        if($menu_key == 'index_edit') {
            ...
            $menu_array[] = array('menu_key' => 'qrcode_list', 'menu_name' => '二维码模板', 'menu_url' => urlAdminMobile('mb_special', 'qrcode_list'));
        } else if ($menu_key == 'special_list') {
            ...
            $menu_array[] = array('menu_key' => 'qrcode_list', 'menu_name' => '二维码模板', 'menu_url' => urlAdminMobile('mb_special', 'qrcode_list'));        
        } else if ($menu_key == 'qrcode_list') {
            $menu_array[] = array('menu_key' => 'index_edit', 'menu_name' => '首页', 'menu_url' => urlAdminMobile('mb_special', 'index'));
            $menu_array[] = array('menu_key' => 'special_list', 'menu_name' => '专题', 'menu_url' => urlAdminMobile('mb_special', 'special_list'));
            $menu_array[] = array('menu_key' => 'qrcode_list', 'menu_name' => '二维码模板', 'menu_url' => 'javascript:;');
        }
        if($menu_key == 'index_edit') {
