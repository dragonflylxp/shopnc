...
<ul class="cp-toast-list">
  <?php if ($output['statistics']['contract'] > 0) {?>
    <li>
      <span>[商城-运营]</span>
      <a target="workspace" href="<?php echo urlAdminShop('contract','applylist');?>" onclick="openItem('shop|contract')"><i class="fa fa-bell-o"></i>有<strong><?php echo $output['statistics']['contract'];?></strong>条消费者保障服务加入申请。</a>
    </li>
  <?php }?>
  <?php if ($output['statistics']['contract_quit'] > 0) {?>
    <li>
      <span>[商城-运营]</span>
      <a target="workspace" href="<?php echo urlAdminShop('contract','quitlist');?>" onclick="openItem('shop|contract')"><i class="fa fa-bell-o"></i>有<strong><?php echo $output['statistics']['contract_quit'];?></strong>条消费者保障服务退出申请。</a>
    </li>
  <?php }?>

  <?php if ($output['statistics']['get_money_apply'] > 0) {?>
    <li>
      <span>[商城-运营]</span>
      <a target="workspace" href="<?php echo urlAdminShop('manager_rebill_check','index');?>" onclick="openItem('shop|manager_rebill_check')"><i class="fa fa-bell-o"></i>有
        <strong><?php echo $output['statistics']['get_money_apply'];?></strong>条管理人实物提现申请需要审核。</a>
    </li>
  <?php }?>
  <?php if ($output['statistics']['get_vrmoney_apply'] > 0) {?>
    <li>
      <span>[商城-运营]</span>
      <a target="workspace" href="<?php echo urlAdminShop('manager_vrbill_check','index');?>" onclick="openItem('shop|manager_vrbill_check')"><i class="fa fa-bell-o"></i>有
        <strong><?php echo $output['statistics']['get_vrmoney_apply'];?></strong>条管理人虚拟提现申请需要审核。</a>
    </li>
  <?php }?>
  <?php if ($output['statistics']['manager_apply'] > 0) {?>
    <li>
      <span>[商城-运营]</span>
      <a target="workspace" href="<?php echo urlAdminShop('manager','manager_apply');?>" onclick="openItem('shop|manager')"><i class="fa fa-bell-o"></i>有<strong><?php echo $output['statistics']['manager_apply'];?></strong>条管理人申请需要审核。</a>
    </li>
  <?php }?>
  <?php if ($output['statistics']['order_offbank'] > 0) {?>
    <li>
      <span>[商城-交易]</span>
      <a target="workspace" href="<?php echo urlAdminShop('order','list');?>" onclick="openItem('shop|order')"><i class="fa fa-bell-o"></i>有<strong><?php echo $output['statistics']['order_offbank'];?></strong>条银行汇款订单需要处理。</a>
    </li>
  <?php }?>
  <?php if ($output['statistics']['order_vr_offbank'] > 0) {?>
    <li>
      <span>[商城-交易]</span>
      <a target="workspace" href="<?php echo urlAdminShop('vr_order','list');?>" onclick="openItem('shop|vr_order')"><i class="fa fa-bell-o"></i>有<strong><?php echo $output['statistics']['order_vr_offbank'];?></strong>条银行汇款虚拟订单需要处理。</a>
    </li>
  <?php }?>
  <?php if ($output['statistics']['order_offbank'] > 0) {?>
  <li>
