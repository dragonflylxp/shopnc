...
class member_buyControl extends mobileMemberControl {
    ...
    public function buy_step1Op() {
        ...
        if(is_file($inc_file)){
            ...
            //top5
            usort($integral_list, function($a,$b){
                $max_a = $a['balance']*$a['sysBuyingRate']; 
                $max_b = $b['balance']*$b['sysBuyingRate'];
                return $max_a >= $max_b ? -1 : 1;
            });
            $integral_list_top5 = array_slice($integral_list, 0,5);
        }

        $buy_list = array();
        $buy_list['store_cart_list'] = $store_cart_list;
        $buy_list['integral_list'] = $integral_list_top5;
    ...
    public function buy_step2Op() {
        ...
        $param['rpt'] = $_POST['rpt'];
        $param['jforderstr'] = $_POST['jforderstr'];
    ...
    public function check_passwordOp() {
        ...
        $member_info = $model_member->getMemberInfoByID($this->member_info['member_id']);
        if($member_info['member_paypwd'] == md5($_POST['password']+'XMzDdG7D94CKm1IxIWQw6g==')) {
            output_data('1');
        } else {
            output_error('密码错误');
        }
    ...
    public function payOp() {
        ...
        if(!empty($_POST['exchange_card_submit'])){
            ...
            $obj->checkOp($order_list,$_POST['exchange_card_submit']);
        }
        ...
        //减去积分抵扣金额
        if ($pay_info['jforderstr']) {
            //格式: type1|points1|amount1#type2|points2|amount2#...
            $jforders = explode('#', $pay_info['jforderstr']);
            foreach($jforders as $k=>$jforder){
                $jfinfo = explode('|', $jforder);
                $pay['pay_diff_amount'] = $pay['pay_diff_amount'] - floatval($jfinfo[2]);
            }    
        } 
        ...
        */
        ...
        $pay['jforderstr'] = $pay_info['jforderstr'];
    ...
    public function check_pd_pwdOp(){
        ...
        if ($buyer_info['member_paypwd'] != '') {
            if ($buyer_info['member_paypwd'] === md5($_POST['password']+'XMzDdG7D94CKm1IxIWQw6g==')) {
                output_data('1');
            }
        }
