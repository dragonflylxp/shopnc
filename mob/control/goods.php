...
class goodsControl extends mobileHomeControl{
    ...
    public function goods_listOp() {
        ...
        $extend = 'goods_common';       //gongbo 20161226
        ...
        if(!empty($_GET['gc_id']) && intval($_GET['gc_id']) > 0) {
            ...
            $condition['goods.goods_barcode'] = $_GET['barcode'];
        ...
        */
        ...
        if (!is_null($goods_list)) {
            ...
            //虚拟商品
            if ($_GET['virtual'] == 1) {
                $condition['goods.is_virtual'] = 1;
            }

            $goods_list = $model_goods->getGoodsListByColorDistinct($condition, $fieldstr, $order, $this->page,0,$extend);
        }
        ...
        if (!empty($key)) {
            ...
            switch ($key) {
                ...
                case '1' :
                    ...
                    $result = 'goods_common.sale_count' . ' ' . $sequence;            //gongbo 20161226
                    break;
                //浏览量
                case '2' :
    ...
    private function _goods_list_extend($goods_list) {
        ...
        foreach ($goods_list as $key => $value) {
            /*
             * 判断 为null输出"" gongbo 1201
             */
            $_goods_jingle = $goods_list[$key]['goods_jingle'];
            if($_goods_jingle==null || empty($_goods_jingle)){
                $goods_list[$key]['goods_jingle'] = '';
            }
            ...
            /*
             * 商品价格保留两位小数   gongbo 1201
             */
            //$goods_list[$key]['goods_price'] = is_float($_p_price)?$_p_price:sprintf("%.2f",$_p_price);
            $goods_list[$key]['goods_price'] = sprintf("%.2f",$goods_list[$key]['goods_price']);
            /*
             * 商品价格保留两位小数   gongbo 1201
             */
            //商品图片url
    ...
    public function goods_detailOp() {
        ...
        if ($dis_id && $goods_detail['is_dis']) {//分销推广
            $condition = array();
            $condition['distri_id'] = $dis_id;
            $model_dis_goods = Model('dis_goods');
            $dis_goods = $model_dis_goods->getDistriGoodsInfo($condition);
            if ($dis_goods['member_id'] && $dis_goods['distri_goods_state'] == 1) setNcCookie('dis_'.$goods_detail['goods_commonid'],$dis_goods['member_id'],3600*24);
        }
        ...
        //-------------start------gongbo 20161226 评论共享-----------
        $goods_eval_list = Model("evaluate_goods")->getEvaluateGoodsList_b(array('geval_goodsid' => $goods_id), null, '3');
        ...
        $goods_detail['goods_info']['goods_salenum'] =  $goods_detail['goods_info']['sale_count'];
        //输出seo
        $seo_param = array();
        $seo_param['name'] = $goods_detail['goods_info']['goods_name'];
    ...
    public function auto_completeOp() {
        $data = Model('search')->autoCompletewap(array('term'=>$_GET['term']));
        foreach ($data as $k => $v) {
            $data[$k] = $v['value'];
        }
