...
class member_paymentControl extends mobileMemberControl {
    ...
    public function pay_newOp() {
        ...
        if (in_array($_GET['payment_code'],array('ghtmixpay','ghtpay','alipay','wxpay_jsapi'))) {
            $model_mb_payment = Model('mb_payment');
            $condition = array();
    ...
    private function _api_pay($order_pay_info) {
        ...
        if ($this->payment_code == 'ghtmixpay') {
            ...
            if($order_pay_info['jforderstr']){
                $jforders = explode('#', $order_pay_info['jforderstr']);
                foreach($jforders as $k=>$jforder){
                    $jfinfo = explode('|', $jforder);
                    $param['jf_orders'][$k]['targetCurrencyCode'] = 'CNY';
                    $param['jf_orders'][$k]['integralType'] = $jfinfo[0];
                    $param['jf_orders'][$k]['integral'] = $jfinfo[1];
                    $param['jf_orders'][$k]['integralAmount'] = $jfinfo[2];
            ...
            $param['store_merchantno'] = $order_pay_info["order_list"][0]["store_merchantno"];
        }
    ...
    private function _get_real_order_info($pay_sn,$rcb_pd_pay = array()) {
        ...
        //减去积分抵扣金额
        if ($result['data']['jforderstr']) {
            //格式: type1|points1|amount1#type2|points2|amount2#...
            $jforders = explode('#', $result['data']['jforderstr']);
            foreach($jforders as $k=>$jforder){
                $jfinfo = explode('|', $jforder);
                $pay_amount = $pay_amount - floatval($jfinfo[2]);
            }
        }
