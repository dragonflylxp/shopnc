...
class roomControl extends mobileHomeControl
{
    ...
    public function store_infoOp()
    {
		...
		else if($zb_config['type']=="3"){
			$living['player']['rtmp']="rtmp://live.cdibuy.com/store/s".$zb_config['sid'];
			$living['player']['hls']="http://live.cdibuy.com/store/s".$zb_config['sid'].".m3u8";
			}
		else{
			$living['player']['rtmp']="rtmp://".$zb_config['link']."/store/s".$zb_config['sid'];
			$living['player']['hls']="http://".$zb_config['link'].":8134/hls-live/store/_definst_/slive/s".$zb_config['sid'].".m3u8";
			}
		$living['notice']=	$zb_config['gonggao'];		
        //店主推荐
        $where = array();
		...
		$living['zbgtype']=Model('zhibo')->getgtype();
		$living['zbgifts']=Model('zhibo')->getgifts();
		$living['zbid']=$zb_config['uid'];
        output_data(array(
...
public function gift_sendOp() {
        //判断是否为导航页面
		$gid=$_REQUEST['gid'];//虚拟动画ID
		$gnum=$_REQUEST['gnum'];//数量
		$key=$_REQUEST['key'];
		$mb_user_token_info =Model('mb_user_token')->getMbUserTokenInfoByToken($key);
        $this->member_info = Model('member')->getMemberInfoByID($mb_user_token_info['member_id']);
		$userid=$this->member_info['member_id'];//赠送用户ID
		$sid=$_REQUEST['sid'];//店铺ID
		$zb_config=Model('zhibo')->zb_config($sid);
		$reid=$zb_config['uid'];
	...
	public function chaxunOp() {
		...
		$mb_user_token_info =Model('mb_user_token')->getMbUserTokenInfoByToken($key);
        $this->member_info = Model('member')->getMemberInfoByID($mb_user_token_info['member_id']);
		$userid=$this->member_info['member_id'];//用户ID
		$condition['member_id'] = $userid; 
		$user=Model('member')->getMemberInfo($condition);
		$return['s']='success'; 
