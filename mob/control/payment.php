...
class paymentControl extends mobileHomeControl{
    ...
    public function returnOp() {
        ...
        if(isset($_GET['redirect']) && $_GET['redirect'] == "1"){
            $callback_info = array('out_trade_no'=>$_GET['order_no'], 'trade_no'=>$_GET['pay_no']);
        }
        else{
    ...
    private function _get_payment_config() {
        ...
        if ($this->payment_code == 'ghtpay') {
            $cfg_file = BASE_PATH.DS.'api'.DS.'payment'.DS.$this->payment_code.DS.$this->payment_code.'.config.php';
            if(is_file($cfg_file)) {
                require($cfg_file);
                return $ghtpay_config;
            }
        }
