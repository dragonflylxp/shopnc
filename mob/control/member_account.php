...
class member_accountControl extends mobileMemberControl {
    ...
    private function _send_bind_mobile_msg() {
        ...
        try {
            ...
            $sms = new Sms();
            $result = $sms->send($this->member_info['member_mobile'],$message,$paramdata);
            if ($result) {
                $data = array();
                $update_data['auth_code'] = $verify_code;
                $update_data['send_acode_time'] = TIMESTAMP;
    ...
    public function modify_paypwd_step5Op() {
        ...
        $model_member = Model('member');
        $update = $model_member->editMember(array('member_id'=>$this->member_info['member_id']),array('member_paypwd'=>md5($_POST['password']+'XMzDdG7D94CKm1IxIWQw6g==')));
        if (!$update) {
            output_error('密码修改失败');
        }
