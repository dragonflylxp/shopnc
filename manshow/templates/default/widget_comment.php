...
$(document).ready(function(){
     ...
     * 评论
     **/
    var url_comment_list = "index.php?con=comment&fun=comment_list&type=<?php echo $output['comment_type'];?>&comment_id=<?php echo $output['comment_id'];?>";
    var url_comment_save = "index.php?con=comment&fun=comment_save&type=<?php echo $output['comment_type'];?>";
    ...
    $("[nc_type=comment_drop]").live('click',function(){
        if(confirm('<?php echo $lang['nc_ensure_del'];?>')) {
            ...
            $.getJSON("index.php?con=comment&fun=comment_drop&type=<?php echo $output['comment_type'];?>", { comment_id: $(this).attr("comment_id")}, function(json){
                if(json.result == "true") {
                    $("#comment_count").microshop_count({type:'-'});
                    item.remove();
                } else {
