...
<div class="eject_con inform-reason">
  ...
  <form id="inform_form" action="<?php echo CIRCLE_SITE_URL;?>/index.php?con=theme_inform&c_id=<?php echo $output['c_id'];?>&t_id=<?php echo $output['t_id']; if(isset($_GET['r_id'])){echo '&r_id='.$_GET['r_id'];}?>" method="post" onsubmit="ajaxpost('inform_form', '<?php echo CIRCLE_SITE_URL;?>/index.php?con=theme_inform&c_id=<?php echo $output['c_id'];?>&t_id=<?php echo $output['t_id']; if(isset($_GET['r_id'])){echo '&r_id='.$_GET['r_id'];}?>', '', 'onerror');">
