...
<div class="circle-share">
  <form method="post" action="<?php echo CIRCLE_SITE_URL;?>/index.php?con=theme_share&c_id=<?php echo $output['c_id'];?>&t_id=<?php echo $output['t_id'];?>" id="share_form" class="feededitor">
...
$(function(){
	...
	$("[nc_type='bindbtn']").bind('click',function(){
	    ...
	    if(isbind == '1'){// Is binding
		    ...
		    window.open(MEMBER_SITE_URL+'/api.php?con=sharebind&type='+data_str.apikey);
		}
	});
	$("[nc_type='finishbtn']").live('click',function(){
	    ...
	    eval( "data_str = "+data_str);
		// check binding
		var url = 'index.php?con=share&fun=checkbind';
