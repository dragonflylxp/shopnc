    ...
    <table class="base-table-style">
      <thead>
        ...
        <tr>
          ...
          <td colspan="5" class="batches"><label for="all" class="handle-btn"><i class="ac0"></i><?php echo $lang['nc_check_all'];?></label>
            <a href="javascript:void(0);" class="handle-btn" nctype="batchbutton" uri="<?php echo CIRCLE_SITE_URL;?>/index.php?con=manage&fun=setmanage&type=yes&c_id=<?php echo $output['c_id'];?>" name="cm_id" confirm="<?php echo $lang['circle_manage_confirm_one'].C('circle_managesum').$lang['circle_manage_confirm_two'];?>" title="<?php echo $lang['circle_manage_title'];?>"><i class="ac1"></i><?php echo $lang['circle_manage'];?></a>
            <a href="javascript:void(0);" class="handle-btn" nctype="batchbutton" uri="<?php echo CIRCLE_SITE_URL;?>/index.php?con=manage&fun=setmanage&type=no&c_id=<?php echo $output['c_id'];?>" name="cm_id" title="<?php echo $lang['circle_manage_cancel'];?>"><i class="ac2"></i><?php echo $lang['circle_manage_cancel'];?></a>
            <a href="javascript:void(0);" class="handle-btn" nctype="batchbutton" uri="<?php echo CIRCLE_SITE_URL;?>/index.php?con=manage&fun=setting&sign=speak&type=no&c_id=<?php echo $output['c_id'];?>" name="cm_id" title="<?php echo $lang['circle_nospeak'];?>"><i class="ac4"></i><?php echo $lang['circle_nospeak'];?></a>
            <a href="javascript:void(0);" class="handle-btn" nctype="batchbutton" uri="<?php echo CIRCLE_SITE_URL;?>/index.php?con=manage&fun=setting&sign=speak&type=yes&c_id=<?php echo $output['c_id'];?>" name="cm_id" title="<?php echo $lang['circle_nospeak_cancel'];?>"><i class="ac3"></i><?php echo $lang['circle_nospeak_cancel'];?></a>
            <a href="javascript:void(0);" class="handle-btn" nctype="batchbutton" uri="<?php echo CIRCLE_SITE_URL;?>/index.php?con=manage&fun=setting&sign=star&type=yes&c_id=<?php echo $output['c_id'];?>" name="cm_id" title="<?php echo $lang['circle_star_title'];?>"><i class="ac8"></i><?php echo $lang['circle_star'];?></a>
            <a href="javascript:void(0);" class="handle-btn" nctype="batchbutton" uri="<?php echo CIRCLE_SITE_URL;?>/index.php?con=manage&fun=setting&sign=star&type=no&c_id=<?php echo $output['c_id'];?>" name="cm_id" title="<?php echo $lang['circle_star_cancel'];?>"><i class="ac9"></i><?php echo $lang['circle_star_cancel'];?></a>
            <a href="javascript:void(0);" class="handle-btn" nctype="batchbutton" uri="<?php echo CIRCLE_SITE_URL;?>/index.php?con=manage&fun=delmember&c_id=<?php echo $output['c_id'];?>" name="cm_id" confirm="<?php echo $lang['nc_ensure_del'];?>" title="<?php echo $lang['nc_delete'];?>"><i class="ac5"></i><?php echo $lang['nc_delete'];?></a></td>
          <?php }?>
          <td colspan="20"><form method="get" class="manage-search-member">
              <input type="hidden" name="con" value="group_manage" />
              <input type="hidden" name="fun" value="member_manage" />
      ...
      <tbody>
        ...
        <tr>
          ...
          <td><dl class="member-base"><dt class="member-name"><a href="<?php echo SHOP_SITE_URL;?>/index.php?con=member_snshome&mid=<?php echo $val['member_id'];?>" target="_blank"><?php echo $val['member_name'];?></a><?php echo memberIdentity($val['is_identity']);?></dt><dd class="member-avatar-s"><img src="<?php echo getMemberAvatarForID($val['member_id']);?>" /><?php if($val['is_star']){echo '<dd class="member-star" title="'.$lang['circle_stat_member'].'"></dd>';}?></dd>
          </dl></td>
...
function settingmember(sign, type, id){
	_uri = "<?php echo CIRCLE_SITE_URL;?>/index.php?con=manage&fun=setting&sign="+sign+"&type="+type+"&c_id=<?php echo $output['c_id'];?>&cm_id="+id;
	ajaxget(_uri);
}

function delmember(id){
	_uri = "<?php echo CIRCLE_SITE_URL;?>/index.php?con=manage&fun=delmember&c_id=<?php echo $output['c_id'];?>&cm_id="+id;
