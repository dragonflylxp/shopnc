...
<div class="theme-detail-poll-content">
  <form method="post" id="poll_form" <?php if(!$output['vote_end'] && !$output['partake']){?>action="<?php echo CIRCLE_SITE_URL;?>/index.php?con=theme&fun=save_votepoll&c_id=<?php echo $output['c_id'];?>&t_id=<?php echo $output['t_id'];?>"<?php }?>>
...
$(function(){
	...
	$('a[nctype="poll_submit"]').click(function(){
		if(_ISLOGIN){
			...
			ajaxpost('poll_form', CIRCLE_SITE_URL+'/index.php?con=theme&fun=save_votepoll&c_id='+c_id+'&t_id='+t_id, '', 'onerror');
		}else{
			login_dialog();		
		}
	});
