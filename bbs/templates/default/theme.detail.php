  ...
  <div class="base-layout mt20">
    <div class="mainbox">
      <div class="theme-detail-header">
        ...
        <div class="theme-info-a"> <a target="_blank" href="<?php echo SHOP_SITE_URL;?>/index.php?con=sns_circle&mid=<?php echo $output['theme_info']['member_id'];?>" nctype="mcard" data-param="{'id':<?php echo $output['theme_info']['member_id'];?>}"><?php echo $output['theme_info']['member_name'];?></a><span class="floor">(<?php echo $lang['circle_landlord'];?>)</span>
        ...
        <div class="theme-info-b">
          ...
          <span class="addtime"><?php echo $lang['circle_posted_in'];?><em><?php echo @date('Y-m-d', $output['theme_info']['theme_addtime']);?></em></span><span class="theme-edit"><i></i><a href="index.php?con=theme&fun=edit_theme&c_id=<?php echo $output['c_id'];?>&t_id=<?php echo $output['t_id'];?>"><?php echo $lang['nc_edit_theme'];?></a></span>
          <?php }?>
        </div>
        <div class="buttons"><a href="Javascript:history.go(-1);" class="back"><i></i><?php echo $lang['circle_return'];?></a>
          <?php if(C('circle_istalk') == 1){?>
          <div class="post-btn"><a href="index.php?con=theme&fun=new_theme&c_id=<?php echo $output['c_id'];?>" class="new-theme"><i></i><?php echo $lang['circle_new_theme'];?></a>
            <div class="sub-menu"><span class="arrow"></span>
              <ul>
                <li class="new-poll"><a href="index.php?con=theme&fun=new_theme&sp=1&c_id=<?php echo $output['c_id'];?>"><i></i><?php echo $lang['circle_new_poll'];?></a></li>
              </ul>
            </div>
          </div>
      ...
      <div class="theme-detail-content">
        ...
        <div class="handle-bar">
          ...
          <div class="manage"> <a href="javascript:void(0);" class="manage-button"><?php echo $lang['circle_theme_manage'];?></a> <span class="manage-content"> <a href="javascript:void(0);" nctype="<?php if($output['theme_info']['is_digest'] == 0){?>themeDigestYes<?php }else{?>themeDigestNo<?php }?>">
            ...
            </a> <a href="<?php echo CIRCLE_SITE_URL;?>/index.php?con=manage&fun=edit_theme&c_id=<?php echo $output['c_id'];?>&t_id=<?php echo $output['t_id'];?>"><?php echo $lang['nc_edit'];?></a> <a href="javascript:void(0);" nctype="themeDelManage"><?php echo $lang['nc_delete'];?></a> </span> </div>
          <?php }?>
          <div class="normal"> <a href="<?php echo CIRCLE_SITE_URL;?>/index.php?con=theme&fun=theme_detail&c_id=<?php echo $output['c_id']?>&t_id=<?php echo $output['t_id'];if($_GET['only_id'] == ''){?>&only_id=<?php echo $val['member_id'];}?>" class="noborder">
        ...
        <li class="reply-info" id="f<?php echo $val['reply_id'];?>">
          <dl>
            ...
            <dd class="handle-bar">
              ...
              <?php }?>
              <div class="normal"> <a href="<?php echo CIRCLE_SITE_URL;?>/index.php?con=theme&fun=theme_detail&c_id=<?php echo $output['c_id']?>&t_id=<?php echo $output['t_id'];if($_GET['only_id'] == ''){?>&only_id=<?php echo $val['member_id'];}?>">
                <?php if($_GET['only_id'] != ''){echo $lang['circle_see_all'];}else{echo $lang['circle_see_TA'];}?>
                </a>
                <a href="<?php echo CIRCLE_SITE_URL?>/index.php?con=theme&fun=reply&c_id=<?php echo $output['c_id'];?>&t_id=<?php echo $output['t_id'];?>&answer_id=<?php echo $val['reply_id'];?>"><?php echo $lang['circle_reply'];?></a>
      ...
      <div class="quick-reply">
        ...
        <form method="post" id="reply_form" action="<?php echo CIRCLE_SITE_URL;?>/index.php?con=theme&fun=save_reply&c_id=<?php echo $output['c_id'];?>&t_id=<?php echo $output['t_id'];?>">
...
$(function(){
	...
	$('.quick-reply').ncUBB({
		...
		UBBform : $('#reply_form'),
		UBBfileuploadurl : 'index.php?con=theme&fun=image_upload&c_id='+c_id,
...
//横高局中比例缩放隐藏显示图片
    ...
    $('#reply_form').validate({
    	...
    	submitHandler:function(form){
    		ajaxpost('reply_form', CIRCLE_SITE_URL+'/index.php?con=theme&fun=save_reply&c_id='+c_id+'&t_id='+t_id, '', 'onerror');
    	},
        rules : {
        	replycontent : {
                required : true
    ...
    $('a[nctype="del_reply"]').click(function(){
    	...
    	showDialog('<?php echo $lang['nc_ensure_del'];?>', 'confirm', '', function(){
    		_uri = "<?php echo CIRCLE_SITE_URL;?>/index.php?con=theme&fun=del_reply&c_id="+c_id+"&t_id="+t_id+"&r_id="+r_id;
    		ajaxget(_uri);
    	});
	});
	...
	$('a[nctype="themeShare"]').click(function(){
		if(_ISLOGIN){
			var _uri = CIRCLE_SITE_URL+"/index.php?con=theme_share&c_id="+c_id+"&t_id="+t_id;
	...
	$('a[nctype="inform"]').click(function(){
		if(_ISLOGIN){
			var _uri = CIRCLE_SITE_URL+"/index.php?con=theme_inform&c_id="+c_id+"&t_id="+t_id;
