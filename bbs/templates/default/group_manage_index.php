...
<div class="base-layout">
  <div class="mainbox">
    ...
    <form id="circle_form" action="<?php echo CIRCLE_SITE_URL;?>/index.php?con=manage&c_id=<?php echo $output['c_id'];?>" method="post" class="base-form-style">
...
$(function(){
	...
	function ajaxFileUpload()
	{
		...
		(
			{
				url:'index.php?con=cut&fun=pic_upload&form_submit=ok&uploadpath=<?php echo ATTACH_CIRCLE;?>/group',
				secureuri:false,
				fileElementId:'c_img',
				dataType: 'json',
				...
				{
					if (data.status == 1){
						ajax_form('cutpic','<?php echo $lang['nc_cut'];?>','index.php?con=cut&fun=pic_cut&filename=<?php echo ATTACH_CIRCLE;?>/group/<?php echo intval($_GET['c_id']);?>.jpg&x=120&y=120&resize=1&url='+data.url,680);
					}else{
						alert(data.msg);
					}
    ...
    $('#circle_form').validate({
    	...
    	submitHandler:function(form){
    		ajaxpost('circle_form', '<?php echo CIRCLE_SITE_URL;?>/index.php?con=manage&c_id=<?php echo $output['c_id'];?>', '', 'onerror');
    	},
        rules : {
        	c_desc : {
            	maxlength : 255
