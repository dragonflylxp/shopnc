      ...
      <tfoot>
        <tr>
          ...
          <th colspan="20"><label for="all" class="handle-btn"><i class="ac0"></i><?php echo $lang['nc_check_all'];?></label>
            <a href="javascript:void(0);" class="handle-btn" nctype="batchbutton" uri="<?php echo CIRCLE_SITE_URL;?>/index.php?con=manage&fun=class_del&c_id=<?php echo $output['c_id'];?>" name="thc_id" confirm="<?php echo $lang['nc_ensure_del'];?>"><i class="ac5"></i><?php echo $lang['nc_delete'];?></a> </th>
        </tr>
      </tfoot>
      <?php }?>
    </table>
...
$(function(){
    ...
    $('a[nctype="add_thclass"]').click(function(){
    	_uri = "<?php echo CIRCLE_SITE_URL;?>/index.php?con=manage&fun=class_add&inajax=1&c_id=<?php echo $output['c_id'];?>";
    	CUR_DIALOG = ajax_form('add_thclass', '<?php echo $lang['circle_tclass_add'];?>', _uri, 520);
	});
});
function edit_thclass(id){
	_uri = "<?php echo CIRCLE_SITE_URL;?>/index.php?con=manage&fun=class_edit&inajax=1&c_id=<?php echo $output['c_id'];?>&thc_id="+id;
...
function del_thclass(id){
	showDialog('<?php echo $lang['nc_ensure_del'];?>', 'confirm', '', function(){
		_uri = "<?php echo CIRCLE_SITE_URL;?>/index.php?con=manage&fun=class_del&inajax=1&c_id=<?php echo $output['c_id'];?>&thc_id="+id;
		ajaxget(_uri);
	});
}
</script>