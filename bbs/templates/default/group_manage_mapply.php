      ...
      <thead>
        ...
        <tr>
          ...
          <td colspan="5" class="batches"><label for="all" class="handle-btn"><i class="ac0"></i><?php echo $lang['nc_check_all'];?></label>
            <a href="javascript:void(0);" class="handle-btn" nctype="batchbutton" uri="<?php echo CIRCLE_SITE_URL;?>/index.php?con=manage_mapply&fun=mapply_pass&c_id=<?php echo $output['c_id'];?>" name="cm_id" confirm="<?php echo $lang['circle_manage_confirm_one'].C('circle_managesum').$lang['circle_manage_confirm_two'];?>" title="<?php echo $lang['circle_manage_title'];?>"><i class="ac1"></i><?php echo $lang['circle_pass'];?></a>
            <a href="javascript:void(0);" class="handle-btn" nctype="batchbutton" uri="<?php echo CIRCLE_SITE_URL;?>/index.php?con=manage_mapply&fun=del&c_id=<?php echo $output['c_id'];?>" name="cm_id" confirm="<?php echo $lang['nc_ensure_del'];?>" title="<?php echo $lang['circle_manage_title'];?>"><i class="ac5"></i><?php echo $lang['nc_delete'];?></a>
          <?php }?>
        </tr>
      </thead>
      <tbody>
...
function mapplyPass(id){
	showDialog('<?php echo $lang['circle_manage_confirm_one'].C('circle_managesum').$lang['circle_manage_confirm_two'];?>', 'confirm', '', function(){
		_uri = "<?php echo CIRCLE_SITE_URL;?>/index.php?con=manage_mapply&fun=mapply_pass&c_id=<?php echo $output['c_id'];?>&cm_id="+id;
		ajaxget(_uri);
	});
}
...
function mapplyDel(id){
	showDialog('<?php echo $lang['nc_ensure_del'];?>', 'confirm', '', function(){
		_uri = "<?php echo CIRCLE_SITE_URL;?>/index.php?con=manage_mapply&fun=del&c_id=<?php echo $output['c_id'];?>&cm_id="+id;
		ajaxget(_uri);
	});
}
</script>