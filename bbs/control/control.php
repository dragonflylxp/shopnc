 ...
 *
 *
 * @copyright  Copyright (c) 2007-2015 shopec Inc. (http://www.shopec.net)
...
class BaseCircleControl{
    ...
    protected function breadcrumd($param = ''){
        $crumd = array(
            ...
            1=>array(
                'link'=>CIRCLE_SITE_URL.'/index.php?con=group&c_id='.$this->c_id,
                'title'=>$this->circle_info['circle_name']
            ),
        );
        if(!empty($this->theme_info)){
            $crumd[2] = array(
                'link'=>CIRCLE_SITE_URL.'/index.php?con=theme&fun=theme_detail&c_id='.$this->c_id.'&t_id='.$this->t_id,
...
class BaseCircleManageControl extends BaseCircleControl{
    ...
    protected  function sidebar_menu($sign, $child_sign=''){
        $menu = array(
                    'index'=>array('menu_name'=>L('circle_basic_setting'), 'menu_url'=>'index.php?con=manage&c_id='.$this->c_id),
                    'member'=>array('menu_name'=>L('circle_member_manage'), 'menu_url'=>'index.php?con=manage&fun=member_manage&c_id='.$this->c_id),
                    'applying'=>array('menu_name'=>L('circle_wait_apply'), 'menu_url'=>'index.php?con=manage&fun=applying&c_id='.$this->c_id),
                    'level'=>array('menu_name'=>L('circle_member_level'), 'menu_url'=>'index.php?con=manage_level&fun=level&c_id='.$this->c_id),
                    'class'=>array('menu_name'=>L('circle_tclass'), 'menu_url'=>'index.php?con=manage&fun=class&c_id='.$this->c_id),
                    'inform'=>array(
                                'menu_name'=>L('circle_inform'),
                                'menu_url'=>'index.php?con=manage_inform&fun=inform&c_id='.$this->c_id,
                                'menu_child'=>array(
                                            'untreated'=>array('name'=>L('circle_inform_untreated'), 'url'=>'index.php?con=manage_inform&fun=inform&c_id='.$this->c_id),
                                            'treated'=>array('name'=>L('circle_inform_treated'), 'url'=>'index.php?con=manage_inform&fun=inform&type=treated&c_id='.$this->c_id)
                                        ),
                            ),
                    'managerapply'=>array('menu_name'=>L('circle_mapply'), 'menu_url'=>'index.php?con=manage_mapply&c_id='.$this->c_id),
                    'friendship'=>array('menu_name'=>L('fcircle'), 'menu_url'=>'index.php?con=manage&fun=friendship&c_id='.$this->c_id)
                );
        if($this->identity == 2){
