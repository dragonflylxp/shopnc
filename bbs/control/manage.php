...
class manageControl extends BaseCircleManageControl{
    ...
    public function indexOp(){
        ...
        if($this->identity == 2){
            @header("location: ".CIRCLE_SITE_URL.'/index.php?con=manage&fun=applying&c_id='.$this->c_id);
        }
        // 会员加入圈子列表
    ...
    public function edit_themeOp(){
        ...
        if(chksubmit()){
            ...
            if($rs){
                ...
                showDialog(L('nc_common_op_succ'), CIRCLE_SITE_URL.'/index.php?con=theme&fun=theme_detail&c_id='.$this->c_id.'&t_id='.$t_id, 'succ');
            }else{
    ...
    public function del_themeOp(){
        ...
        showDialog(L('nc_common_op_succ'), CIRCLE_SITE_URL.'/index.php?con=group&c_id='.$this->c_id, 'succ');
    }
    /**
     * 删除回复
     */
