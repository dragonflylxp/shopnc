...
<!--                按照时间搜索-->
                <th>起止时间</th>
                <td class="w240">
                    <input type="text" id="stime" name="stime" class="text w70" value="<?php echo $_GET['stime'];?>"><label class="add-on"><i class="icon-calendar"></i></label>&nbsp;&#8211;&nbsp;
                    <input type="text" id="etime" name="etime" class="text w70" value="<?php echo $_GET['etime'];?>"><label class="add-on"><i class="icon-calendar"></i></label></td>
<!--                按类别搜索-->
                <th>结算状态</th>
                <td class="w100">
                    <select name="stage">
        ...
        <?php  if (count($output['list_manager_bill'])>0) { ?>
            <?php foreach($output['list_manager_bill'] as $val) { ?>
                <tr class="bd-line">
                    ...
                    <?php }elseif($val['state'] == 5){ ?>
                        <td><span><div class="btn-4">审核未通过</div></span></td>
            <?php } ?>
                </tr>
            <?php } ?>
        <?php } else { ?>
...
<script language="javascript">
    $(function(){
        ...
        $('.apply_change').on('click',function(){
            ...
            $.ajax({
                type:"POST",
                url:'<?php echo urlMember('manager_index','apply');?>',
                async:true,
                dataType:"json",
                data:{id: _ob_id},
