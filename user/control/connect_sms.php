...
class connect_smsControl extends BaseLoginControl{
    ...
    public function registerOp(){
        ...
        if (chksubmit() && strlen($phone) == 11 && strlen($captcha) == 6){
            ...
            if(!empty($referral_code)){
                $member = $model_member->getMemberInfo(array('code'=> $referral_code));//检查手验证码是否存在
                if(empty($member)) {
                    showDialog('推荐码无效','','error');
                }
            }
    ...
    public function loginOp(){
        if (checkSeccode($_POST['nchash'],$_POST['captcha'])){
            ...
            $model_member = Model('member');
            //$member = $model_member->getMemberInfo(array('member_mobile'=> $phone));//检查手机号是否已被注册
            $member_all = $model_member->getMemberAllInfo(array('member_mobile'=> $phone));//检查手机号下面所有账号
            $member = array();
