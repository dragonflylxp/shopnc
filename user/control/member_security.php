...
class member_securityControl extends BaseMemberControl {
    ...
    public function send_bind_emailOp() {
        ...
        \shopec\Lib::messager()->send($_POST["email"],$subject,$message);
        showDialog('验证邮件已经发送至您的邮箱，请于24小时内登录邮箱并完成验证！如果您始终未收到邮件，请于60秒后重新发送','index.php?con=member_security&fun=index','succ','',10);
    }
    ...
    public function modify_paypwdOp() {
        ...
        $update = $model_member->editMember(array('member_id'=>$_SESSION['member_id']),array('member_paypwd'=>md5($_POST['password']+'XMzDdG7D94CKm1IxIWQw6g=='),'is_payment'=>1));
    ...
    public function send_modify_mobileOp() {
        ...
        if(C('sms.smsNumber') == 1){
        	$paramdata['sendtime']  = date('Y-m-d H:i',TIMESTAMP);
        	$paramdata['verifycode']  = $verify_code;
        	$paramdata['template']  = C('dysms.binding');
        }             
        $sms = new Sms();
        $result = $sms->send($_GET["mobile"],$message,$paramdata);
        if ($result) {
            $data = array();
