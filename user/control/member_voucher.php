...
class member_voucherControl extends BaseMemberControl
{
    ...
    public function voucher_dhqOp(){
        $db =Model('exchange_card');
        $list = $db->getExchangeCardByMemberId($_SESSION['member_id']);

        Tpl::output('list', $list);
        $this->profile_menu('voucher_dhq');
        Tpl::showpage('member_voucher.dhq');
    }
    ...
    private function profile_menu($menu_key = '')
    {
        $menu_array = array(
            ...
            3 => array(
                'menu_key' => 'voucher_dhq',
                'menu_name' => '我的兑换券',
                'menu_url' => 'index.php?con=member_voucher&fun=voucher_dhq'
            )
        );
