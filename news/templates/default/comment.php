...
$(document).ready(function(){
    var url_comment_list = "<?php echo CMS_SITE_URL.DS;?>index.php?con=comment&fun=comment_list&type=<?php echo $_GET['con'];?>&comment_object_id=<?php echo $output['detail_object_id'];?>&comment_all=<?php echo $output['comment_all'];?>";
    $("#btn_comment_submit").click(function(){
        if($("#input_comment_message").val() != '') {
        $.post("<?php echo CMS_SITE_URL.DS.'index.php?con=comment&fun=comment_save';?>", $("#add_form").serialize(),
            function(data){
                if(data.result == 'true') {
    ...
    $("[nctype=comment_drop]").live('click',function(){
        if(confirm('<?php echo $lang['nc_ensure_del'];?>')) {
            ...
            $.post("index.php?con=comment&fun=comment_drop&type=<?php echo $_GET['con'];?>", { comment_id: $(this).attr("comment_id")}, function(json){
                if(json.result == "true") {
                    item.remove();
                } else {
    ...
    $('#comment_list').on('click', '[nctype="btn_comment_quote_publish"]', function() {
        ...
        $.post("<?php echo CMS_SITE_URL.DS.'index.php?con=comment&fun=comment_save';?>", {comment_id:comment_id, comment_object_id:comment_object_id, comment_type:comment_type, comment_message:comment_message},
            function(data){
                if(data.result == 'true') {
                    $("#input_comment_message").val("");
                    $("#comment_list").load(url_comment_list);
    ...
    $('#comment_list').on('click', '[nctype="comment_up"]', function() {
        ...
        $.post("<?php echo CMS_SITE_URL.DS.'index.php?con=comment&fun=comment_up';?>", {comment_id:comment_id},
            function(data){
                if(data.result == 'true') {
...
<div id="cms_comment_flag" class="article-comment-title">
  <h3></h3>
  <span><?php echo $lang['cms_comment1'];?><a href="<?php echo CMS_SITE_URL.DS;?>index.php?con=<?php echo $_GET['con'];?>&fun=<?php echo $_GET['con'];?>_comment_detail&<?php echo $_GET['con'];?>_id=<?php echo $output['detail_object_id'];?>"><em><?php echo $output[$_GET['con'].'_detail'][$_GET['con'].'_comment_count'];?></em></a><?php echo $lang['cms_comment2'];?><em><?php echo $output[$_GET['con'].'_detail'][$_GET['con'].'_click'];?></em><?php echo $lang['cms_comment3'];?></span> </div>
<form id="add_form" action="" class="article-comment-form">
  <input id="input_comment_type" name="comment_type" type="hidden" value="<?php echo $_GET['con'];?>" />
