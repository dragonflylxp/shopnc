...
<form action="<?php echo CMS_SITE_URL;?>/index.php?con=share&fun=share_save&type=<?php echo $_GET['con'];?>" method="post" id="share_form" class="feededitor">
  <input type="hidden" value="<?php echo $output['detail_object_id'];?>" name="share_id">
  </input>
  <input type="hidden" value="<?php echo $output[$_GET['con'].'_detail'][$_GET['con'].'_title'];?>" name="share_title">
  </input>
  <input type="hidden" value="<?php echo getCMSArticleImageUrl($output[$_GET['con'].'_detail'][$_GET['con'].'_attachment_path'], $output[$_GET['con'].'_detail'][$_GET['con'].'_image']);?>" name="share_image">
  </input>
