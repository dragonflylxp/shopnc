...
$(document).ready(function(){
    ...
    $("#picture_image_upload").fileupload({
        dataType: 'json',
            url: "<?php echo CMS_SITE_URL.DS.'index.php?con=publish&fun=article_image_upload';?>",
            add: function(e,data) {
    ...
    $("[nctype='btn_drop_picture_image']").live("click",function(){
        ...
        $.getJSON("<?php echo CMS_SITE_URL.DS.'index.php?con=publish&fun=article_image_drop';?>", { image_name: image_name }, function(result){
                if(result.status == "success") {
                    image_object.remove();
                } else {
                    showError(result.error);
    ...
    $("[nctype='btn_goods_add_commit']").live('click', function() {
        ...
        if(goods_list.find("dl").length < 3) {
            ...
            var image_name = $(this).attr("image_name");
            if(url != '') {
                $.getJSON("<?php echo CMS_SITE_URL.DS.'index.php?con=api&fun=goods_info_by_url';?>", { url: url}, function(data){
                    if(data.result == "true") {
...
<div class="cms-content mt10">
  <form id="add_form" method="post" action="index.php?con=publish&fun=publish_picture_save">
