	...
	%>
		...
		<li class="<%if(order_group_list[i].pay_amount){%>green-order-skin<%}else{%>gray-order-skin<%}%> <%if(i>0){%>mt10<%}%>">
				...
				<div class="nctouch-order-item">
					...
					<div class="nctouch-order-item-head">
						...
						<%if (orderlist[j].ownshop){%>
							...
							<a class="store" href="<%=ApiUrl%>/index.php?con=store&store_id=<%=orderlist[j].store_id%>"><i class="icon"></i><%=orderlist[j].store_name%></a>

						<%}else{%>

							<a href="<%=ApiUrl%>/index.php?con=store&fun=index&store_id=<%=orderlist[j].store_id%>" class="store"><i class="icon"></i><%=orderlist[j].store_name%><i class="arrow-r"></i></a>
					...
					<div class="nctouch-order-item-con">
						...
						<a href="<%=ApiUrl%>/index.php?con=member_order&fun=order_detail&order_id=<%=orderlist[j].order_id%>">
			...
			<%if(order_group_list[i].pay_amount && order_group_list[i].pay_amount>0){%>
				...
				<a href="javascript:;" data-paySn="<%=order_group_list[i].pay_sn%>" class="btn-l check-payment">订单支付<em>（￥<%= p2f(order_group_list[i].pay_amount) %>）</em></a>
...
<script type="text/javascript" src="<?php echo MOBILE_TEMPLATES_URL;?>/js/list/order_list.js"></script>
