<?php defined('Inshopec') or exit('Access Invalid!');?>

<link rel="stylesheet" type="text/css" href="<?php echo MOBILE_TEMPLATES_URL;?>/css/nctouch_member.css">

<style type="text/css">

  .w20h li {

    width: 20%;

}

</style>

</head>

<body>



<header id="header" class="fixed">

  <div class="header-wrap">

    <div class="header-l"> <a href="javascript:history.go(-1)"> <i class="back"></i> </a> </div>

    <div class="header-title">

      <h1>店铺统计</h1>

    </div>

  <div class="header-r"> <a id="header-nav" href="javascript:void(0);"><i class="more"></i><sup></sup></a> </div>

  </div>

   <?php include template('layout/seller_toptip');?>



</header>

<div class="nctouch-main-layout">

 <div id="fixed_nav" class="nctouch-single-nav">

    <ul id="filtrate_ul" class="w20h">

      <li class="selected"><a href="javascript:void(0);" >店铺概况</a></li>

      <li><a href="<?php echo urlMobile('seller_stat','goodslist');?>" >店铺统计</a></li>

      <li><a href="<?php echo urlMobile('seller_stat','storeflow');?>" >店铺流量</a></li>

      <li><a href="<?php echo urlMobile('seller_stat','goodsflow');?>" >商品流量</a></li>

      <li><a href="<?php echo urlMobile('seller_stat','hotgoods');?>" >热销排行</a></li>

     

    </ul>

  </div>

    <div class="alert " >

      <ul class="mt5">

        <li>1、符合以下任何一种条件的订单即为有效订单：1）采用在线支付方式支付并且已付款；2）采用货到付款方式支付并且交易已完成</li>

        <li>2、以下关于订单和订单商品近7天统计数据的依据为：从昨天开始最近7天的有效订单</li>

        <li>3、手机端商家中心报表统计最近7天数据,如需查看更多请登录pc端商家中心进行查看</li>

        </ul>

    </div>

    <div class="alert alert-info mt5" style="clear:both;">

        <ul class="mt5">

    <li>

      <span class="h30" style="display:block;">

        <i title="店铺从昨天开始最近7天有效订单的总金额" class="tip icon-question-sign"></i>

        近7天下单金额：<strong><?php echo $output['statnew_arr']['orderamount'].$lang['currency_zh'];?></strong>

      </span>

    <span class="h30" style="display:block;">

      <i title="店铺从昨天开始最近7天有效订单的会员总数" class="tip icon-question-sign"></i>

      近7天下单会员数：<strong><?php echo $output['statnew_arr']['ordermembernum'];?></strong>

    </span>

    <span class="h30" style="display:block;">

      <i title="店铺从昨天开始最近7天有效订单的总订单数" class="tip icon-question-sign"></i>

      近7天下单量：<strong><?php echo $output['statnew_arr']['ordernum'];?></strong>

    </span>

    <span class="h30" style="display:block;">

      <i title="店铺从昨天开始最近7天有效订单的总商品数量" class="tip icon-question-sign"></i>

      近7天下单商品数：<strong><?php echo $output['statnew_arr']['ordergoodsnum'];?></strong>

    </span>

 

      <span class="h30" style="display:block;">

        <i title="店铺从昨天开始最近7天有效订单的平均每个订单的交易金额" class="tip icon-question-sign"></i>

        平均客单价：<strong><?php echo $output['statnew_arr']['avgorderamount'].$lang['currency_zh'];?></strong>

      </span>

      <span class="h30" style="display:block;">

        <i title="店铺从昨天开始最近7天有效订单商品的平均每个商品的成交价格" class="tip icon-question-sign"></i>

        平均价格：<strong><?php echo $output['statnew_arr']['avggoodsprice'].$lang['currency_zh'];?></strong>

      </span>

      <span class="h30" style="display:block;">

        <i title="店铺所有商品的总收藏次数" class="tip icon-question-sign"></i>

        商品收藏量：<strong><?php echo $output['statnew_arr']['gcollectnum'];?></strong>

      </span>

      <span class="h30" style="display:block;">

        <i title="店铺拥有商品的总数（仅计算商品种类，不统计库存）" class="tip icon-question-sign"></i>

        商品总数：<strong><?php echo $output['statnew_arr']['goodsnum'];?></strong>

      </span>



      <span class="h30" style="display:block;">

        <i title="店铺总收藏次数" class="tip icon-question-sign"></i>

        店铺收藏量：<strong><?php echo $output['statnew_arr']['store_collect'];?></strong>

      </span>

      <span class="w400 fl h30" style="display:block;">

        <i title="店铺从昨天开始最近7天有效订单的下单频繁的时间点" class="tip icon-question-sign"></i>

        下单高峰期：<strong><?php echo ($t = $output['statnew_arr']['hothour'])?$t:'暂无';?></strong>

      </span>

    </li>

  </ul>

  <div style="clear:both;"></div>

</div>

<div id="container"></div>



<div class="w450  ">

  <div class="alert alert-info" style="margin-bottom:0px;"><strong>建议推广商品</strong>

    &nbsp;<i title="统计店铺从昨天开始7日内热销商品前30名，建议推广以下商品，提升推广回报率" class="tip icon-question-sign"></i>

  </div>

    <table class="ncsc-default-table">

      <thead>

        <tr class="sortbar-array">

          <th class="align-center">序号</th>

          <th class="align-center">商品名称</th>

          <th class="align-center">销量</th>

        </tr>

      </thead>

      <tbody id="datatable">

        <?php if (!empty($output['goodstop30_arr']) && is_array($output['goodstop30_arr'])) { ?>

            <?php foreach($output['goodstop30_arr'] as $k=>$v) { ?>

            <tr class="bd-line">

              <td class="w50"><?php echo $k+1;?></td>

              <td class="tl">

                <span class="over_hidden w340 h20">

                  <a href="<?php echo urlShop('goods', 'index', array('goods_id' => $v['goods_id']));?>" target="_blank"><?php echo $v['goods_name'];?></a>

                </span>

              </td>

              <td class="w50"><?php echo $v['ordergoodsnum'];?></td>

            </tr>

            <?php } ?>

        <?php } else { ?>

        <tr>

          <td colspan="20" class="norecord"><div class="warning-option"><i class="icon-warning-sign"></i><span>暂无符合条件的数据记录</span></div></td>

        </tr>

        <?php } ?>

      </tbody>

    </table>

</div>



<div class="w450 ">

    <div class="alert alert-info" style="margin-bottom:0px;"><strong>同行热卖</strong>

    &nbsp;<i title="拥有相同经营类目同行店铺热销商品推荐，了解行业热卖便于调整商品结构" class="tip icon-question-sign"></i>

  </div>

    <table class="ncsc-default-table">

      <thead>

        <tr class="sortbar-array">

          <th class="align-center">序号</th>

          <th class="align-center">商品名称</th>

          <th class="align-center">销量</th>

        </tr>

      </thead>

      <tbody id="datatable">

        <?php if (!empty($output['othergoodstop30_arr']) && is_array($output['othergoodstop30_arr'])) { ?>

            <?php foreach($output['othergoodstop30_arr'] as $k=>$v) { ?>

            <tr class="bd-line">

              <td class="w50"><?php echo $k+1;?></td>

              <td class="tl">

                <span class="over_hidden w340 h20">

                  <a href="<?php echo urlMobile('goods', 'index', array('goods_id' => $v['goods_id']));?>" target="_blank"><?php echo $v['goods_name'];?></a>

                </span>

              </td>

              <td class="w50"><?php echo $v['ordergoodsnum'];?></td>

            </tr>

            <?php } ?>

        <?php } else { ?>

        <tr>

          <td colspan="20" class="norecord"><div class="warning-option"><i class="icon-warning-sign"></i><span>暂无符合条件的数据记录</span></div></td>

        </tr>

        <?php } ?>

      </tbody>

    </table>

</div>

</div>







<script type="text/javascript" src="<?php echo MOBILE_TEMPLATES_URL;?>/js/jquery.js"></script>

<script type="text/javascript" src="<?php echo MOBILE_TEMPLATES_URL;?>/js/list/highcharts.js"></script>

<script>

$(function(){



    $("#header").on("click", "#header-nav",

    function() {

        if ($(".nctouch-nav-layout").hasClass("show")) {

            $(".nctouch-nav-layout").removeClass("show")

        } else {

            $(".nctouch-nav-layout").addClass("show")

        }

    });

 $('#container').highcharts(<?php echo $output['stattoday_json'];?>);

});

</script>

